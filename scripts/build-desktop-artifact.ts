...
import { fromYaml } from "@trumbo-code/shared/schemaYaml";
import { HostProcessPlatform } from "@trumbo-code/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@trumbo-code/shared/relayAuth";
...
import { resizeLinuxIconWithPngJs } from "./lib/linux-icon-resize.ts";
...
const DESKTOP_APP_ID = "dev.trumbo.trumbo-code";
...
export class UnsupportedCrossPlatformDesktopBuildError extends Schema.TaggedErrorClass<UnsupportedCrossPlatformDesktopBuildError>()(
  "UnsupportedCrossPlatformDesktopBuildError",
  {
  ...
  override get message(): string {
    return `Cannot build ${this.targetPlatform} desktop artifacts on a ${this.hostPlatform} host. Use a matching runner (CI macOS/Linux jobs, or \`vp run dist:desktop:${this.targetPlatform === "mac" ? "dmg:arm64" : this.targetPlatform}\` on native hardware).`;
  }
}
...
interface StagePackageJson {
  ...
  readonly trumboCodeCommitHash: string;
...
export class InvalidAppleTeamIdError extends Schema.TaggedErrorClass<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `TRUMBO_CODE_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedErrorClass<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "TRUMBO_CODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedErrorClass<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "TRUMBO_CODE_CLERK_PUBLISHABLE_KEY or TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedErrorClass<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "TRUMBO_CODE_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export const isLinuxIconResizeError = Schema.is(LinuxIconResizeError);
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.TRUMBO_CODE_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.TRUMBO_CODE_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.TRUMBO_CODE_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
// retains collected top-level dependencies. Trumbo Code desktop auth uses the device-code flow and
// does not ship Clerk passkey native binaries; the staging step was removed. The artifact resolver
// is retained for build-config tests and downstream tooling that introspects the platform matrix.
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "TRUMBO_CODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("TRUMBO_CODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "TRUMBO_CODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("TRUMBO_CODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("TRUMBO_CODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("TRUMBO_CODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("TRUMBO_CODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("TRUMBO_CODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("TRUMBO_CODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("TRUMBO_CODE_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("TRUMBO_CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
    Config.option,
  ),
  ...
  wslPrebuild: Config.string("TRUMBO_CODE_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
    ...
    Effect.catch(() =>
      HostProcessPlatform.pipe(
        Effect.flatMap((platform) =>
          platform === "win32"
            ? // @effect-diagnostics-next-line globalErrorInEffectFailure:off - contained: caught immediately by the pngjs fallback below.
              Effect.fail(new Error("ImageMagick convert is not available on Windows hosts."))
            : resize("convert"),
        ),
      ).pipe(
        Effect.catch(() =>
          Effect.try({
            try: () => {
              resizeLinuxIconWithPngJs(sourcePng, targetPng, iconSize);
            },
            catch: (cause) =>
...
export function collectBundledClientAssetRefs(
  indexHtml: string,
  entryJsContent: string | undefined,
  ...
  for (const match of indexHtml.matchAll(/\b(?:src|href)=["']([^"']+)["']/g)) {
    ...
    const normalizedRef = value.split("#")[0]?.split("?")[0] ?? "";
    if (!normalizedRef) continue;
    refs.add(normalizedRef.replace(/^\/+/, ""));
  }

  if (entryJsContent) {
    for (const match of entryJsContent.matchAll(/assets\/[A-Za-z0-9_.-]+\.(?:js|css|wasm)/g)) {
      refs.add(match[0]);
    }
  }
...
function validateBundledClientAssets(clientDir: string) {
  return Effect.gen(function* () {
    ...
    const entryScriptMatch = indexHtml.match(/\bsrc=["']\/(assets\/index-[^"']+\.js)["']/);
    const entryScriptRelativePath = entryScriptMatch?.[1];
    const entryJsContent =
      entryScriptRelativePath && (yield* fs.exists(path.join(clientDir, entryScriptRelativePath)))
        ? yield* fs.readFileString(path.join(clientDir, entryScriptRelativePath))
        : undefined;
    const refs = collectBundledClientAssetRefs(indexHtml, entryJsContent);
    ...
    for (const ref of refs) {
      ...
      const assetPath = path.join(clientDir, normalizedRef);
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("TRUMBO_CODE_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Trumbo Code (Nightly)"
    : (desktopPackageJson.productName ?? "Trumbo Code");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "Trumbo-Code-${version}-${arch}.${ext}",
    directories: {
  ...
  if (platform === "mac") {
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "Trumbo Code",
          schemes: ["trumbo-code", "trumbo-dev"],
        },
      ],
      ...(macPasskeySigning
        ? {
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "trumbo-code",
      icon: "icons",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "trumbo-code",
        },
      },
    };
  }
...
// node-pty's prebuilds/linux-<arch>/ with a trumbo-code marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / TRUMBO_CODE_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(
    path.join(prebuildDir, "trumbo-code-wsl-node-pty.json"),
    `${markerJson}\n`,
  );
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const fs = yield* FileSystem.FileSystem;
  const hostPlatform = yield* HostProcessPlatform;
  const hostBuildPlatform = detectHostBuildPlatform(hostPlatform);
  if (hostBuildPlatform && hostBuildPlatform !== options.platform) {
    return yield* new UnsupportedCrossPlatformDesktopBuildError({
      hostPlatform: hostBuildPlatform,
  ...
  const stageRoot = yield* mkdir({
    prefix: `trumbo-code-desktop-${options.platform}-stage-`,
  });
  ...
  // Trumbo Code desktop auth uses the device-code flow and does not require
  // passkey entitlements. macOS signed builds still do plain code signing via
  // the CSC_* / Apple API key env vars; passkey entitlements are only added
  // when an explicit passkey RP domain (or Clerk publishable key) + Apple
  // team id + provisioning profile are all provided. Missing passkey config is
  // treated as "no passkey entitlements", not a build failure.
  const configuredMacPasskeySigning =
    options.platform === "mac" && options.signed
      ? yield* Effect.try({
          try: () => resolveMacPasskeySigningConfiguration(loadRepoEnv({ repoRoot })),
          catch: (cause) =>
            isMacPasskeySigningConfigurationError(cause)
              ? cause
              : MacPasskeySigningConfigurationResolutionError.fromCause(cause),
        }).pipe(
          Effect.catchIf(
            (cause): cause is MacPasskeySigningConfigurationError =>
              isMacPasskeySigningConfigurationError(cause) &&
              (cause._tag === "MissingMacPasskeyDomainConfigurationError" ||
                cause._tag === "MissingMacPasskeyProvisioningProfileError" ||
                cause._tag === "InvalidAppleTeamIdError"),
            () => Effect.void,
          ),
        )
      : undefined;
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    trumboCodeCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Trumbo Code desktop build",
    author: "Trumbo",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @trumbo-code/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: TRUMBO_CODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: TRUMBO_CODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription(
      "Build arch, for example arm64/x64/universal (env: TRUMBO_CODE_DESKTOP_ARCH).",
    ),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: TRUMBO_CODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: TRUMBO_CODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: TRUMBO_CODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: TRUMBO_CODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: TRUMBO_CODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: TRUMBO_CODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: TRUMBO_CODE_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription(
      "Mock update server port (env: TRUMBO_CODE_DESKTOP_MOCK_UPDATE_SERVER_PORT).",
    ),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: TRUMBO_CODE_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Trumbo Code."),
