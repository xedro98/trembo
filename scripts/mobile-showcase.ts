...
const ANDROID_PACKAGE = "dev.trumbo.trumbo-code.dev";
const APP_SCHEME = "trumbo-dev";
...
const IOS_APP_PATH = NodePath.join(
  MOBILE_ROOT,
  ".showcase/ios-derived-data/Build/Products/Debug-iphonesimulator/TrumboCodeDev.app",
);
...
Usage:
  pnpm --filter @trumbo-code/mobile screenshots [options]
...
async function buildIos(): Promise<string> {
  ...
  await runCommand(
    ...
    [
      "-workspace",
      NodePath.join(MOBILE_ROOT, "ios/TrumboCodeDev.xcworkspace"),
      "-scheme",
      "TrumboCodeDev",
      "-configuration",
      "Debug",
