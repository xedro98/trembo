...
const devServerInput = {
  mode: "dev:server",
  trumboCodeHome: "/tmp/trumbo-dev-runner",
  noBrowser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit TRUMBO_CODE_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "TRUMBO_CODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "TRUMBO_CODE_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults TRUMBO_CODE_HOME to ~/.trumbo-code when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TRUMBO_CODE_HOME, path.resolve(NodeOS.homedir(), ".trumbo-code"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TRUMBO_CODE_HOME, path.resolve("/tmp/custom-t3"));
        assert.equal(env.TRUMBO_CODE_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.TRUMBO_CODE_NO_BROWSER, "1");
        assert.equal(env.TRUMBO_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.TRUMBO_CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.TRUMBO_CODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            TRUMBO_CODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          trumboCodeHome: undefined,
        ...
        assert.equal(env.TRUMBO_CODE_MODE, "web");
        assert.equal(env.TRUMBO_CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TRUMBO_CODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom trumboCodeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.TRUMBO_CODE_HOME, path.resolve("/tmp/my-t3"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TRUMBO_CODE_MODE, undefined);
        assert.equal(env.TRUMBO_CODE_NO_BROWSER, undefined);
        assert.equal(env.TRUMBO_CODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.TRUMBO_CODE_PORT, "13773");
    ...
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { TRUMBO_CODE_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, [
          "TRUMBO_CODE_PORT_OFFSET",
          "TRUMBO_CODE_DEV_INSTANCE",
        ]);
