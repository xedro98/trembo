// @effect-diagnostics nodeBuiltinImport:off globalDate:off - This host-side fixture creates an isolated local Trumbo environment.
...
export const SHOWCASE_PROJECT_ID = "trumbo-code";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/trumbo-code\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
...
const PROJECT_FAVICONS = {
  "trumbo-code": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    id: "trumbo-code",
    title: "Trumbo Code",
    directory: "trumbo-code",
    repositoryUrl: "https://github.com/xedro98/trumbo-code.git",
    favicon: PROJECT_FAVICONS["trumbo-code"],
  },
  {
    id: "react",
    title: "React",
...
async function seedTrumboCodeWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "trumbo-code", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(
    NodePath.join(workspaceRoot, "favicon.svg"),
    PROJECT_FAVICONS["trumbo-code"],
  );
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the Trumbo Code mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedTrumboCodeWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
