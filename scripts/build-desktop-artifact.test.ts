...
import { HostProcessArchitecture, HostProcessPlatform } from "@trumbo-code/shared/hostProcess";
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Trumbo Code");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Trumbo Code (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                TRUMBO_CODE_DESKTOP_UPDATE_REPOSITORY: "xedro98/trumbo-code",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "xedro98/trumbo-code",
              },
            }),
          ),
        ),
      ...
      resolveDesktopRuntimeDependencies(
        {
          ...
          "@trumbo-code/contracts": "workspace:*",
          "@trumbo-code/shared": "workspace:*",
          "@trumbo-code/ssh": "workspace:*",
          "@trumbo-code/tailscale": "workspace:*",
          effect: "catalog:",
          electron: "41.5.0",
        },
        {
  ...
  it.effect("reports Linux icon resize failures after CLI and pngjs fallbacks", () => {
    const commands: Array<{ readonly command: string; readonly args: ReadonlyArray<string> }> = [];

    return Effect.gen(function* () {
      const error = yield* stageLinuxIconSize("source.png", "target.png", 512, false).pipe(
        Effect.provide(iconResizeSpawnerLayer(commands, [1, 1])),
        Effect.flip,
      );
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      TRUMBO_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/trumbo-code.provisionprofile",
      TRUMBO_CODE_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });

    assert.deepStrictEqual(configuration, {
      appId: "dev.trumbo.trumbo-code",
      teamId: "ABC1234567",
      rpDomains: ["example.clerk.accounts.dev"],
      provisioningProfilePath: "/tmp/trumbo-code.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      TRUMBO_CODE_APPLE_TEAM_ID: "ABC1234567",
      TRUMBO_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/trumbo-code.provisionprofile",
      TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.dev.trumbo.trumbo-code</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      TRUMBO_CODE_APPLE_TEAM_ID: "ABC1234567",
      TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "TRUMBO_CODE_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      TRUMBO_CODE_APPLE_TEAM_ID: "ABC1234567",
      TRUMBO_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/trumbo-code.provisionprofile",
      TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          TRUMBO_CODE_APPLE_TEAM_ID: "ABC1234567",
          TRUMBO_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/trumbo-code.provisionprofile",
          TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      TRUMBO_CODE_APPLE_TEAM_ID: "ABC1234567",
      TRUMBO_CODE_MACOS_PROVISIONING_PROFILE: "/tmp/trumbo-code.provisionprofile",
      TRUMBO_CODE_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(
      ...
      "TRUMBO_CODE_CLERK_PUBLISHABLE_KEY is invalid.",
    );
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "dev.trumbo.trumbo-code");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/trumbo-code.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        { name: "Trumbo Code", schemes: ["trumbo-code", "trumbo-dev"] },
      ]);
      ...
      const resolved = yield* resolveBuildOptions({
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                TRUMBO_CODE_DESKTOP_SKIP_BUILD: "true",
                TRUMBO_CODE_DESKTOP_KEEP_STAGE: "true",
                TRUMBO_CODE_DESKTOP_SIGNED: "true",
                TRUMBO_CODE_DESKTOP_VERBOSE: "true",
                TRUMBO_CODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
