# Trumbo Connect Relay

> [!WARNING]
> Trumbo Connect is currently in private beta. Join the waitlist in the app under Settings > Trumbo Connect.

The relay is the hosted control plane for Trumbo Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal Trumbo Code traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [Trumbo Connect architecture overview](../../docs/cloud/trumbo-code-connect-auth-flow.html) for the larger system
design.
...
- Linking Trumbo Code environments to a cloud account.
...
```sh
vp run --filter trumbo-code-relay deploy
```
...
```sh
vp run --filter trumbo-code-relay deploy -- --stage prod
vp run --filter trumbo-code-relay deploy -- --env-file .env.local
```
...
- `TRUMBO_PLATFORM_URL` (defaults to `https://platform.trumbo.dev`; the relay introspects user bearer tokens via `${TRUMBO_PLATFORM_URL}/api/me`)
- `APNS_ENVIRONMENT`
- `APNS_TEAM_ID`
- `APNS_KEY_ID`
- `APNS_BUNDLE_ID`
...
derived public URL from the same environment for downstream desktop, CLI, and hosted web builds.

See:

- [Trumbo Connect Clerk Setup](../../docs/cloud/t3-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [Trumbo Connect Architecture Overview](../../docs/cloud/trumbo-code-connect-auth-flow.html) for the full link,
