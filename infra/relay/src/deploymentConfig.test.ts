...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("trumbo-code-relay-traces", "prod")).toBe(
      "trumbo-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("trumbo-code-relay-traces", "dev_julius")).toBe(
      "trumbo-code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "t3coderelay.com", hash)).toBe(
      "prod-abcdef0123456789.trumbo-codecoderelay.com",
    );
