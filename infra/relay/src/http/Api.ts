...
import { httpHeaderRedactionLayer } from "@trumbo-code/shared/httpObservability";
...
import { normalizeRelayIssuer } from "@trumbo-code/shared/relayJwt";
    ...
    return {
      clientBearer: (httpEffect, { credential }) =>
        Effect.gen(function* () {
          ...
          const verified = yield* verifyPlatformToken(config, token).pipe(
            Effect.provide(FetchHttpClient.layer),
            Effect.tapError((error) =>
              Effect.annotateCurrentSpan(
                "relay.auth.platform_verification_failure",
                platformVerificationFailureReason(
          ...
          if (!verified.sub) {
            yield* Effect.annotateCurrentSpan({
              "relay.auth.platform_verification_failure": "missing_subject",
            });
  ...
  Effect.fnUntraced(function* (handlers) {
    ...
    return handlers.handle(
      ...
      Effect.fn("relay.api.token.exchangeDpopAccessToken")(function* (args) {
        ...
        yield* Effect.annotateCurrentSpan({
          "relay.auth.mode": "platform_bearer_token_exchange",
        ...
        const verified = yield* verifyPlatformToken(config, args.payload.subject_token).pipe(
          Effect.catch(() => relayAuthInvalidError("invalid_bearer")),
        );
        if (!verified.sub) {
          return yield* relayAuthInvalidError("invalid_bearer");
...
class PlatformTokenVerificationFailed extends Schema.TaggedErrorClass<PlatformTokenVerificationFailed>()(
  "PlatformTokenVerificationFailed",
  {
    cause: Schema.Defect(),
  },
) {
  override get message(): string {
    return "Trumbo platform token verification failed";
  }
}
...
function platformVerificationFailureReason(cause: unknown): string {
  if (typeof cause === "string" && cause.length > 0) {
    return safeAuthFailureReason(cause);
  }
...
/**
 * Verify a Trumbo platform bearer token by introspecting it against the
 * platform's `/api/me` endpoint. The platform issues opaque Better Auth
 * session tokens (not JWTs), so the relay can't decode them locally — it asks
 * the platform to authenticate the bearer and return the user id, which becomes
 * the relay account id (`sub`). This replaces the legacy Clerk JWT verification.
 */
function verifyPlatformToken(
  config: RelayConfiguration.RelayConfiguration["Service"],
  token: string,
): Effect.Effect<
  { readonly sub: string; readonly mode: "platform_bearer" },
  PlatformTokenVerificationFailed | HttpClientError.HttpClientError | Schema.SchemaError,
  HttpClient.HttpClient
> {
  return Effect.gen(function* () {
    const client = yield* HttpClient.HttpClient;
    const request = HttpClientRequest.get(`${config.platformBaseUrl}/api/me`).pipe(
    ...
    const response = yield* client
      ...
      .pipe(
        Effect.catch(() =>
          Effect.fail(new PlatformTokenVerificationFailed({ cause: "request_failed" })),
        ),
      );
    if (response.status !== 200) {
      return yield* Effect.fail(new PlatformTokenVerificationFailed({ cause: "unauthorized" }));
    }
    const PlatformTokenResponse = Schema.Struct({
      id: Schema.optionalKey(Schema.Unknown),
      userId: Schema.optionalKey(Schema.Unknown),
      sub: Schema.optionalKey(Schema.Unknown),
    });
    const body = yield* HttpClientResponse.schemaBodyJson(PlatformTokenResponse)(response);
    const sub = String(body.id ?? body.userId ?? body.sub ?? "");
    if (!sub) {
      return yield* Effect.fail(new PlatformTokenVerificationFailed({ cause: "missing_subject" }));
    }
    return { sub, mode: "platform_bearer" as const };
  }).pipe(
    Effect.withSpan("verify_platform_bearer_token", {
...
export function verifyRelayClientBearerToken(
  ...
  return verifyPlatformToken(config, token);
}
