...
import { RelayEnvironmentAuth } from "@trumbo-code/contracts/relay";
...
const relaySettings: RelayConfiguration.RelayConfiguration["Service"] = {
  ...
  apns: {
    ...
    bundleId: "com.example.trumbo-code",
    environment: "sandbox",
  },
...
/** HttpClient mock that responds to `${platformBaseUrl}/api/me` with a JSON
 *  user body ({ id }) for a valid token, 401 for "bad-token". */
function platformAuthHttpClient() {
  return HttpClient.make((req) =>
    Effect.sync(() => {
      const url = new URL(req.url);
      const auth = req.headers.authorization ?? "";
      if (url.pathname === "/api/me" && auth === "Bearer platform-token") {
        return HttpClientResponse.fromWeb(
          req,
          // @effect-diagnostics-next-line preferSchemaOverJson:off - test mock serializes a fake HTTP response body.
          new Response(JSON.stringify({ id: "user-1", email: "u@trumbo.dev" }), {
            status: 200,
            headers: { "content-type": "application/json" },
          }),
        );
      }
      return HttpClientResponse.fromWeb(req, new Response("unauthorized", { status: 401 }));
    }),
  );
}

describe("relay client authentication", () => {
  it.effect("verifies a Trumbo platform bearer token via /api/me introspection", () =>
    Effect.gen(function* () {
      const verified = yield* verifyRelayClientBearerToken(relaySettings, "platform-token").pipe(
        Effect.provideService(HttpClient.HttpClient, platformAuthHttpClient()),
      );
      expect(verified).toEqual({ sub: "user-1", mode: "platform_bearer" });
    }),
  );

  it.effect("rejects an invalid platform bearer token", () =>
    Effect.gen(function* () {
      const result = yield* verifyRelayClientBearerToken(relaySettings, "bad-token").pipe(
        Effect.provideService(HttpClient.HttpClient, platformAuthHttpClient()),
        Effect.matchEffect({
          onFailure: () => Effect.succeed({ _tag: "Left" as const }),
          onSuccess: () => Effect.succeed({ _tag: "Right" as const }),
        }),
      );
