...
export const RelayObservability = Effect.gen(function* () {
  ...
  const traces = yield* Axiom.Dataset("RelayTracesDataset", {
    name: relayResourceNameForStage("trumbo-code-relay-traces", stage),
    kind: "otel:traces:v1",
    description: "Trumbo Code relay Worker HTTP request spans.",
    retentionDays: 30,
    useRetentionPeriod: true,
  });

  const workerIngestToken = yield* Axiom.ApiToken("RelayWorkerAxiomIngestToken", {
    name: relayResourceNameForStage("trumbo-code-relay-otel-ingest", stage),
  ...
  const mobileIngestToken = yield* Axiom.ApiToken("RelayMobileAxiomIngestToken", {
    name: relayResourceNameForStage("trumbo-code-mobile-otel-ingest", stage),
    description: "Owned by Alchemy. Scoped OTLP ingest token for Trumbo Code mobile spans.",
    datasetCapabilities: Output.map(traces.name, (dataset) => ({
      [dataset]: { ingest: ["create" as const] },
    })),
  });
  ...
  const clientIngestToken = yield* Axiom.ApiToken("RelayClientAxiomIngestToken", {
    name: relayResourceNameForStage("trumbo-code-relay-client-otel-ingest", stage),
  ...
  yield* Axiom.View("RelayRecentSpansView", {
    name: relayResourceNameForStage("trumbo-code-relay-recent-spans", stage),
...
export const makeRelayTraceLayer = (input: {
  ...
  Layer.effect(
    ...
    OtlpTracer.make({
      ...
      resource: {
        serviceName: "trumbo-code-relay-worker",
        attributes: {
