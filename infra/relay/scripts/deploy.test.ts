...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "TRUMBO_CODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nTRUMBO_CODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "trumbo-code-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "trumbo-code-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "TRUMBO_CODE_MOBILE_OTLP_TRACES_DATASET=trumbo-code-mobile-traces-dev",
        "TRUMBO_CODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=trumbo-code-relay-client-traces-dev",
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "TRUMBO_CODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "TRUMBO_CODE_RELAY_URL=https://old.example.test",
          "TRUMBO_CODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "TRUMBO_CODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "TRUMBO_CODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
