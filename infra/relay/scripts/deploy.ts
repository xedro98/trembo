...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    TRUMBO_CODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    TRUMBO_CODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    TRUMBO_CODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("TRUMBO_CODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("TRUMBO_CODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "TrumboCodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Trumbo Code relay through Alchemy."));
