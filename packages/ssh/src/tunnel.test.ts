...
describe("ssh tunnel scripts", () => {
  it("builds the remote trumbo-code runner with npx and npm fallbacks", () => {
    const script = buildRemoteTrumboRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "T3_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec trumbo-code "$@"');
    assert.include(script, "exec npx --yes 'trumbo-code@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'trumbo-code@latest' -- \"$@\"");
    assert.include(script, "could not install 'trumbo-code@latest'");
  ...
  it("does not hard-code a remote node engine range", () => {
    const script = buildRemoteTrumboRunnerScript();

    assert.include(script, "T3_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote trumbo-code runner", () => {
    const script = buildRemoteTrumboRunnerScript({
      packageSpec: "trumbo-code@nightly; touch /tmp/t3-owned",
    });

    assert.include(script, "exec npx --yes 'trumbo-code@nightly; touch /tmp/t3-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'trumbo-code@nightly; touch /tmp/t3-owned' -- \"$@\"",
    );
    assert.notInclude(script, "exec npx --yes trumbo-code@nightly; touch /tmp/t3-owned");
  });

  it("builds the remote trumbo-code runner with a node script override", () => {
    const script = buildRemoteTrumboRunnerScript({
  ...
  it("uses the remote trumbo-code runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(buildRemoteLaunchScript(), "Remote Trumbo server did not become ready");
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "trumbo-code@nightly" }),
      "trumbo-code@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "trumbo-code@nightly" }),
      "trumbo-code@nightly",
    );
    assert.include(
