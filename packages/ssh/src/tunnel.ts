...
} from "@trumbo-code/shared/httpReadiness";
import * as NetService from "@trumbo-code/shared/Net";
import { extractJsonObject, fromLenientJson } from "@trumbo-code/shared/schemaJson";
import { satisfiesSemverRange } from "@trumbo-code/shared/semver";
...
export interface RemoteTrumboRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemoteTrumboRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemoteTrumboRunnerOptions | undefined) {
...
if command -v trumbo-code >/dev/null 2>&1; then
  exec trumbo-code "$@"
fi
...
printf 'Remote host is missing the trumbo-code CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@T3_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.trumbo-code/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.trumbo-code"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-trumbo-code.sh"
RUNNER_NEXT="$STATE_DIR/run-trumbo-code.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env TRUMBO_CODE_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@T3_READY_TIMEOUT_MS@@"; then
    printf 'Remote Trumbo server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.trumbo-code/ssh-launch/@@T3_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.trumbo-code"
RUNNER_FILE="$STATE_DIR/run-trumbo-code.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteTrumboRunnerScript(input?: RemoteTrumboRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "trumbo-code@latest");
...
export function buildRemoteNodeEnvScript(input?: RemoteTrumboRunnerOptions): string {
  return stripTrailingNewlines(
...
export function buildRemoteLaunchScript(input?: RemoteTrumboRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    T3_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    T3_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteTrumboRunnerScript(input)),
...
export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteTrumboRunnerOptions,
): string {
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteTrumboRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteTrumboRunnerOptions;
...
>()("@trumbo-code/ssh/tunnel/SshEnvironmentManager") {
