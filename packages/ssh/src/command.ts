...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@trumbo-code/contracts";
...
export function resolveRemoteTrumboCliPackageSpec(input: {
  ...
  if (!input.isDevelopment && PUBLISHABLE_T3_VERSION_PATTERN.test(appVersion)) {
    return `trumbo-code@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "trumbo-code@nightly";
  }

  return input.updateChannel === "nightly" ? "trumbo-code@nightly" : "trumbo-code@latest";
}
