...
import {
  ...
  resolveRemoteTrumboCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote trumbo-code package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteTrumboCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "trumbo-code@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteTrumboCliPackageSpec({
