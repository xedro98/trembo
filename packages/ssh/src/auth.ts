...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@trumbo-code/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "trumbo-code-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Trumbo Code re-runs ssh with a cached password
...
printf 'Trumbo Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Trumbo Code re-runs\r
...
[Console]::Error.WriteLine("Trumbo Code ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "trumbo-code-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "trumbo-code" }),
  };
});
