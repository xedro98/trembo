...
export interface DesktopPreviewWebviewConfig {
  /** `persist:trumbo-code-preview` (or whatever the desktop chose). */
  partition: string;
  /**
...
export type TrumboSubscriptionTier = "free" | "pro" | "max" | "ultra";

export type TrumboSubscriptionStatus = "active" | "trialing" | "past_due" | "canceled" | "none";

export interface TrumboPlanRateLimitWindow {
  readonly used: number;
  readonly limit: number;
  readonly resetsAtSec: number;
}

export interface TrumboPlanUsage {
  readonly fiveHour?: TrumboPlanRateLimitWindow;
  readonly daily?: TrumboPlanRateLimitWindow;
  readonly weekly?: TrumboPlanRateLimitWindow;
}

/** @deprecated Static entitlement caps; live usage is in `TrumboSubscription.usage`. */
export interface TrumboSubscriptionRateLimits {
  readonly requestsPerHour: number;
  readonly requestsPerDay: number;
  readonly concurrentSessions: number;
}

export interface TrumboSubscription {
  readonly tier: TrumboSubscriptionTier;
  readonly status: TrumboSubscriptionStatus;
  readonly periodEnd?: string;
  readonly displayName?: string;
  readonly usage?: TrumboPlanUsage;
  /** @deprecated Prefer `usage` for live plan limits. */
  readonly rateLimits?: TrumboSubscriptionRateLimits;
}

export interface TrumboAuthUser {
  readonly id: string;
...
export type TrumboAuthStatus = "signed-out" | "signing-in" | "signed-in" | "error";

export interface TrumboAuthState {
  readonly status: TrumboAuthStatus;
  readonly user?: TrumboAuthUser;
  readonly subscription?: TrumboSubscription;
  readonly errorMessage?: string;
  /** Present while status === "signing-in" during the RFC 8628 device-code flow. */
  readonly deviceCode?: TrumboDeviceCodeRequest;
  /**
   * Trumbo platform access token (opaque Better Auth session token). Present
   * when status === "signed-in". The desktop renderer forwards this to the
   * Trumbo Connect relay auth layer (in place of the legacy Clerk JWT) so
   * relay/platform calls authenticate with the Trumbo account session. Not
   * present for hosted-web builds (they use same-origin session cookies).
   */
  readonly accessToken?: string;
}

export interface TrumboDeviceCodeRequest {
...
export interface DesktopTrumboAuthBridge {
  getState: () => Promise<TrumboAuthState>;
  startSignIn: () => Promise<TrumboDeviceCodeRequest>;
...
export interface DesktopBridge {
  ...
  /**
   * Trumbo account auth + subscription surface. Present iff the renderer is
   * hosted by the Electron desktop build; hosted web builds authenticate via
   * same-origin session cookies on platform.trumbo.dev instead.
   */
  trumboAuth?: DesktopTrumboAuthBridge;
}
