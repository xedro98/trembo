...
export const TrumboSettings = makeProviderSettingsSchema(
  {
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("trumbo").pipe(
      Schema.annotateKey({
        title: "CLI binary path",
        description:
          "Path to the Trumbo CLI binary. Leave empty to use `trumbo` from PATH, or Bun dev mode when a CLI workspace is configured.",
        providerSettingsForm: { placeholder: "trumbo", clearWhenEmpty: "omit" },
      }),
    ),
    cliCwd: TrimmedString.pipe(
      Schema.withDecodingDefault(Effect.succeed("")),
      Schema.annotateKey({
        title: "CLI workspace",
        description:
    ...
    enableAgentTeams: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({
        title: "Agent teams",
        description:
          "Enable multi-agent team tools (team_spawn_teammate, team_task, etc.) in Trumbo CLI sessions.",
        providerSettingsForm: { hidden: true },
      }),
    ),
    enableSpawnAgent: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(true)),
      Schema.annotateKey({
        title: "Spawn subagents",
        description: "Allow spawn_agent delegated subtasks in Trumbo CLI sessions.",
        providerSettingsForm: { hidden: true },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
  ...
  {
    order: ["binaryPath", "cliCwd"],
  },
);
export type TrumboSettings = typeof TrumboSettings.Type;
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Trumbo Code spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    trumbo: TrumboSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
  }).pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const TrumboSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(TrimmedString),
  cliCwd: Schema.optionalKey(TrimmedString),
  enableAgentTeams: Schema.optionalKey(Schema.Boolean),
  enableSpawnAgent: Schema.optionalKey(Schema.Boolean),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      trumbo: Schema.optionalKey(TrumboSettingsPatch),
    }),
  ),
