...
}).annotate({ description: "Links an authenticated cloud user to a Trumbo environment." });
...
>()("@trumbo-code/contracts/relay/RelayClientPrincipal") {}
...
>()("@trumbo-code/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Trumbo Connect user.",
  ),
);
...
/**
 * Subject token type for an opaque Trumbo platform bearer token (a Better Auth
 * session token minted by the platform.trumbo.dev device-code flow). The relay
 * introspects these by calling `${platformBaseUrl}/api/me` rather than decoding
 * a JWT, because the platform issues opaque session tokens, not JWTs.
 */
export const RelayPlatformSubjectTokenType =
...
export const RelayDpopAccessTokenRequest = Schema.Struct({
  ...
  subject_token: TrimmedNonEmptyString.annotate({
    description: "Trumbo platform bearer token for the signed-in cloud user.",
  }),
  subject_token_type: Schema.Literals([RelayPlatformSubjectTokenType, RelayJwtSubjectTokenType]),
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Trumbo Code Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Trumbo environments, connecting authorized clients, and publishing agent activity.",
  );
