...
export const CLOUDFLARED_PATH_ENV_NAME = "TRUMBO_CODE_CLOUDFLARED_PATH";
...
export class RelayClient extends Context.Service<RelayClient, RelayClientShape>()(
  "@trumbo-code/shared/relayClient",
) {}
  ...
  const installUnlocked = Effect.fn("cloudflared.installUnlocked")(function* (
    ...
    if (!releaseAsset) {
      return yield* new RelayClientInstallError({
        ...
        message: `Trumbo Code does not provide a managed relay client binary for ${platform}-${arch}.`,
      });
    }
