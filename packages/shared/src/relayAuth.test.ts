...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.trumbo.dev")),
    ).toBe("clerk.trumbo.dev");
    expect(clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.trumbo.dev"))).toBe(
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.trumbo.dev/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(isAllowedClerkFrontendApiHostname("clerk.trumbo.dev", "clerk.trumbo.dev")).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.trumbo.dev")).toBe(false);
    expect(isAllowedClerkFrontendApiHostname("nested.clerk.trumbo.dev", "clerk.trumbo.dev")).toBe(
      false,
    );
  });
});
