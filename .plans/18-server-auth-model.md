...
Define the long-term server auth architecture for Trumbo Code before first-class remote environments ship.
...
Every privileged surface of the Trumbo server must go through the same auth policy engine:
...
```text
Participants:
  UserShell     = npx trumbo-code launcher
...
DesktopMain -> Tunnel : publish local Trumbo endpoint
...
Participants:
  ...
  RemoteTrumbo      = remote Trumbo server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify trumbo-code availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : trumbo-code remote launch --json
RemoteHost -> RemoteTrumbo : start or reuse server
RemoteTrumbo --> RemoteHost : port + environment metadata
...
note over RemoteTrumbo : policy = RemoteReachablePolicy
note over DesktopMain,RemoteTrumbo : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteTrumbo : POST /api/auth/bootstrap via forwarded port
RemoteTrumbo -> RemoteTrumbo : validate bootstrap grant
RemoteTrumbo -> RemoteTrumbo : create browser session
RemoteTrumbo --> Frontend : Set-Cookie: session=...

Frontend -> RemoteTrumbo : GET /ws + authenticated cookie
RemoteTrumbo --> Frontend : websocket accepted
```
