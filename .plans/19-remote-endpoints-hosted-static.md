...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Trumbo Tunnel
- users can generate a hosted pairing link for `platform.trumbo.dev`
...
This plan intentionally leaves the paid Trumbo cloud tunnel fabric out of scope. It defines the OSS foundation that Trumbo Tunnel should later plug into.
...
- `https://platform.trumbo.dev` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `platform.trumbo.dev` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://platform.trumbo.dev/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `platform.trumbo.dev`.
...
- `platform.trumbo.dev` works as a real client shell.
...
- `platform.trumbo.dev` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `platform.trumbo.dev`.
...
## Phase 4: Future Trumbo Tunnel Provider
...
The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Trumbo Code should only see an `AdvertisedEndpoint`.
