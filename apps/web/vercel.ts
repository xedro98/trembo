...
const ROUTER_HOST = "platform.trumbo.dev";
const HOSTED_WEB_CHANNEL_COOKIE = "t3code_web_channel";
const LATEST_ORIGIN = "https://latest.platform.trumbo.dev";
const NIGHTLY_ORIGIN = "https://nightly.platform.trumbo.dev";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @trumbo-code/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@trumbo-code/scripts...' --filter '@trumbo-code/web...'",
  routes: [
    {
