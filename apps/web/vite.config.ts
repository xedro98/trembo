...
import { DEFAULT_TRUMBO_RELAY_URL } from "../../packages/shared/src/relayUrl.ts";
...
const configuredRelayUrl = repoEnv.VITE_TRUMBO_CODE_RELAY_URL?.trim() || DEFAULT_TRUMBO_RELAY_URL;
...
const sourcemapEnv = process.env.TRUMBO_CODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: TRUMBO_CODE_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.TRUMBO_CODE_BUNDLED_DEV?.trim().toLowerCase();
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_TRUMBO_CODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
