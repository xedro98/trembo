...
import { subscribeCommandPaletteOpen, type CommandPaletteOpenTarget } from "../commandPaletteBus";
...
interface CommandPaletteOpenIntent {
  readonly kind: CommandPaletteOpenTarget;
}
...
type CommandPaletteUiAction =
  ...
  | { readonly _tag: "OpenWithIntent"; readonly intent: CommandPaletteOpenIntent | null }
...
function reduceCommandPaletteUiState(
  ...
  switch (action._tag) {
    ...
    case "OpenWithIntent":
      return { open: true, openIntent: action.intent };
...
export function CommandPalette({ children }: { children: ReactNode }) {
  ...
  const openAddProject = useCallback(
    () => dispatch({ _tag: "OpenWithIntent", intent: { kind: "add-project" } }),
    [],
  );
  ...
  useEffect(() => {
    return subscribeCommandPaletteOpen((options) => {
      if (options?.open === "add-project") {
        dispatch({ _tag: "OpenWithIntent", intent: { kind: "add-project" } });
        return;
      }
      if (options?.open === "new-thread-in") {
        dispatch({ _tag: "OpenWithIntent", intent: { kind: "new-thread-in" } });
        return;
      }
      dispatch({ _tag: "OpenWithIntent", intent: null });
    });
  }, []);
...
function renderProjectActionIcon(project: Project): ReactNode {
  return (
    <ProjectFavicon
...
function OpenCommandPaletteDialog(props: {
  ...
  const openSourceControlSettings = useCallback(() => {
    setOpen(false);
    openSettingsModal("source-control");
  }, [setOpen]);
  ...
  useLayoutEffect(() => {
    ...
    clearOpenIntent();
    const submenu = actionItems.find(
      (item): item is CommandPaletteSubmenuItem =>
        item.kind === "submenu" && item.value === "action:new-thread-in",
    );
    if (submenu) {
      pushView(submenu);
    }
    // Only react to the open intent; actionItems is rebuilt each render.
