...
import { CHAT_LIST_ANCHOR_OFFSET } from "@trumbo-code/shared/chatList";
import { projectScriptCwd, projectScriptRuntimeEnv } from "@trumbo-code/shared/projectScripts";
import { truncate } from "@trumbo-code/shared/String";
import { nextTerminalId, resolveTerminalSessionLabel } from "@trumbo-code/shared/terminalLabels";
...
import { rewriteTeamPrompt, TEAM_COMMAND_USAGE } from "../lib/teamCommand";
import {
...
import { buildTemporaryWorktreeBranchName } from "@trumbo-code/shared/git";
...
import { TrumboAuthRequiredScreen } from "./trumbo-auth/TrumboAuthRequiredScreen";
import { resolveTrumboThreadAccessBlock } from "./trumbo-auth/trumboThreadAccess";
...
function ChatViewContent(props: ChatViewProps) {
      ...
      items.push({
        ...
        actions: (
          <>
            ...
            <Button size="xs" variant="outline" onClick={() => openSettingsModal("connections")}>
              Connections
            </Button>
          </>
        ),
  ...
  const trumboAuthState = useTrumboAuthState();
  const trumboAccessBlock = useMemo(
    () =>
      resolveTrumboThreadAccessBlock({
        isNativeDesktop: isNativeTrumboDesktop(),
        authState: trumboAuthState,
        threadError,
        providerMessage: activeProviderStatus?.message,
      }),
    [activeProviderStatus?.message, threadError, trumboAuthState],
  );
  const showTrumboAuthScreen = trumboAccessBlock.kind !== "none";
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    const teamRewrite = rewriteTeamPrompt(trimmed);
    if (teamRewrite.kind === "usage") {
      toastManager.add(
        stackedThreadToast({
          type: "info",
          title: "Team command",
          description: TEAM_COMMAND_USAGE,
        }),
      );
      promptRef.current = "";
      clearComposerDraftContent(composerDraftTarget);
    ...
    const basePromptForSend = teamRewrite.kind === "rewritten" ? teamRewrite.prompt : promptForSend;
    ...
    const messageTextWithContexts = appendElementContextsToPrompt(
      appendTerminalContextsToPrompt(basePromptForSend, composerTerminalContextsSnapshot),
  ...
  const onProviderModelSelect = useCallback(
    (instanceId: ProviderInstanceId, model: string) => {
      ...
      // Surface a strict resolution failure instead of silently substituting
      // a different model: if the picked slug is not part of the instance's
      // option list, selecting it would quietly fall back to the first option
      // and the user would think the pick worked.
      const selectionEntry = deriveProviderInstanceEntries(providerStatuses).find(
        (candidate) => candidate.instanceId === instanceId,
      );
      if (selectionEntry) {
        const strictOptions = getAppModelOptionsForInstance(settings, selectionEntry);
        if (!resolveSelectableModel(selectionEntry.driverKind, model, strictOptions)) {
          toastManager.add({
            type: "warning",
            title: "Model unavailable",
            description: `${model} is not available on this provider instance yet. Try refreshing or pick another model.`,
          });
          scheduleComposerFocus();
          return;
        }
  ...
  const onEditUserMessage = useCallback(
    async (messageId: MessageId, messageText: string) => {
      ...
      if (phase === "running" || isSendBusy || isConnecting) {
        setThreadError(activeThread.id, "Interrupt the current turn before editing a message.");
        return;
      }

      // Revert directly WITHOUT confirmation dialog — the user explicitly
      // chose to edit, so the intent is clear. The revert removes the old
      // message and everything after it from the timeline.
      setIsRevertingCheckpoint(true);
      setThreadError(activeThread.id, null);
      const result = await revertThreadCheckpoint({
        environmentId,
      ...
      if (result._tag === "Failure" && !isAtomCommandInterrupted(result)) {
        ...
        setThreadError(
          activeThread.id,
          error instanceof Error ? error.message : "Failed to revert for edit.",
        );
        return;
      }

      // After revert completes, populate the composer with the original text.
      // The old message is now gone from the timeline — only the new edit
      // will appear when the user submits.
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      >
        ...
        {/* Error banner */}
        {!showTrumboAuthScreen ? <ProviderStatusBanner status={activeProviderStatus} /> : null}
        {!showTrumboAuthScreen ? (
          <ThreadErrorBanner
            error={threadError}
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          {showTrumboAuthScreen ? (
            <TrumboAuthRequiredScreen block={trumboAccessBlock} />
          ) : (
            <>
              {/* Chat column */}
