...
import { TrumboAgentsModelPicker } from "./TrumboAgentsModelPicker";
import {
  ModelEsque,
...
const EMPTY_MODEL_OPTIONS: ReadonlyArray<ModelEsque> = [];
...
export const ProviderModelPicker = memo(function ProviderModelPicker(props: {
  ...
  const selectedInstanceOptions =
    props.modelOptionsByInstance.get(activeInstanceId) ?? EMPTY_MODEL_OPTIONS;
  ...
  useEffect(() => {
    ...
    const shouldAllowOverlayScroll = (target: EventTarget | null) => {
      return (
        ...
        target.closest("[data-model-picker-content], [data-model-picker-scroll]") !== null
      );
    };
  ...
  const fallbackCatalog = useMemo(
    () =>
      selectedInstanceOptions.map((option) => ({
        id: option.slug,
        name: option.name,
      })),
  ...
  if (useAgentsStylePicker) {
    return (
      <TrumboAgentsModelPicker
        instanceId={activeInstanceId}
        value={props.model}
        onChange={(model) => handleInstanceModelChange(activeInstanceId, model)}
        ...
        getModelDisabledReason={mergedGetModelDisabledReason}
      />
    );
  }
