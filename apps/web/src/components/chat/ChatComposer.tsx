...
import { serializeComposerFileLink } from "@trumbo-code/shared/composerTrigger";
import { createModelSelection, normalizeModelSlug } from "@trumbo-code/shared/model";
import {
  memo,
  useCallback,
  useEffect,
...
import type { UnifiedSettings } from "@trumbo-code/contracts/settings";
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const unlockedSelectedProvider =
    ...
    providerInstanceEntries.find((entry) => entry.enabled)?.driverKind ??
    (isNativeTrumboDesktop()
      ? ProviderDriverKind.make("trumbo")
  ...
  const selectedInstanceId = useMemo<ProviderInstanceId>(() => {
    ...
    if (explicitSelectedInstanceId) {
      const explicitMatch = providerInstanceEntries.find(
        (entry) => entry.instanceId === explicitSelectedInstanceId && entry.enabled,
      );
      if (explicitMatch) {
        return explicitMatch.instanceId;
      }
    }
    ...
    return (
      ...
      activeThreadModelSelection?.instanceId ??
      activeProjectDefaultModelSelection?.instanceId ??
      ProviderInstanceId.make(isNativeTrumboDesktop() ? "trumbo" : "codex")
    );
  }, [
    activeProjectDefaultModelSelection?.instanceId,
    activeThread?.session?.providerInstanceId,
    ...
    if (composerTrigger.kind === "slash-command") {
      const builtInSlashCommandItems = [
        ...
        },
        {
          ...
          command: "team",
          label: "/team",
          description: "Start a coordinated multi-agent team for a task",
        },
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        if (item.command === "team") {
          const applied = applyPromptReplacement(trigger.rangeStart, trigger.rangeEnd, "/team ", {
            expectedText: snapshot.value.slice(trigger.rangeStart, trigger.rangeEnd),
          });
          if (applied) {
            setComposerHighlightedItemId(null);
  ...
  return (
    ...
    >
      <div
        className={cn(
          "group rounded-[3px] p-px transition-colors duration-200",
      ...
      >
        <div
          ...
          className={cn(
            "input-surface rounded-[3px] transition-[border-color,background-color] duration-200",
            isDragOverComposer ? "border-primary/70 bg-accent/45" : null,
