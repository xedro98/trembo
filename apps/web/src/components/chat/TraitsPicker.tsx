...
export const TraitsPicker = memo(function TraitsPicker({
  ...
  for (const descriptor of descriptors) {
    ...
    if (ultrathinkPromptControlled && descriptor.id === primarySelectDescriptor?.id) {
      label = "Reasoning · Ultrathink";
    } else if (descriptor.type === "boolean") {
      if (descriptor.id === "fastMode") {
        label = descriptor.currentValue === true ? "Fast" : "Normal";
      } else if (descriptor.id === "thinking") {
        label = descriptor.currentValue === true ? "Thinking · On" : "Thinking · Off";
      } else {
        label = `${descriptor.label} · ${descriptor.currentValue === true ? "On" : "Off"}`;
      }
    } else {
      const current = getProviderOptionCurrentLabel(descriptor);
      if (current) {
        // Keep the option family visible so "High" reads as thinking/reasoning level.
        const shortLabel =
          descriptor.id === "reasoningEffort" ||
          descriptor.id === "effort" ||
          descriptor.id === "reasoning"
      ...
      <MenuTrigger
        render={
          <Button
            ...
            title="Thinking level and model options"
            className={cn(
              "min-w-0 max-w-44 shrink justify-start overflow-hidden whitespace-nowrap px-2 text-muted-foreground/70 hover:text-foreground/80 sm:max-w-52 sm:px-3 [&_svg]:mx-0",
              triggerClassName,
            )}
          />
        }
