...
import { parseScopedThreadKey } from "@trumbo-code/client-runtime/environment";
import { resolveChatListAnchoredEndSpace } from "@trumbo-code/shared/chatList";
import {
  createContext,
  Fragment,
  memo,
...
import { type TimestampFormat } from "@trumbo-code/contracts/settings";
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  return (
    <div className="group flex flex-col gap-1 border-b border-border/40 pb-4 pt-4 first:pt-0">
      ...
      <div className="flex w-full items-center justify-end gap-2 text-xs tabular-nums opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover:opacity-100">
        <Tooltip>
        ...
        <div className="flex items-center gap-0.5">
          ...
          {canRevertAgentWork && (
            <EditUserMessageButton
              ...
              messageText={displayedUserMessage.visibleText}
            />
          )}
...
function EditUserMessageButton({
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={
          <Button
            ...
            variant="ghost"
            disabled={activity.isRevertingCheckpoint || activity.isWorking}
            onClick={() => ctx.onEditUserMessage(messageId, messageText)}
      ...
      <TooltipPopup side="top">Edit and resubmit</TooltipPopup>
    </Tooltip>
  );
}
...
function AssistantTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  return (
    <>
      <div className="relative min-w-0 border-b border-border/40 px-1 pb-4 pt-4 first:pt-0">
        <ChatMarkdown
          text={messageText}
          cwd={ctx.markdownCwd}
