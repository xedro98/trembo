import type { ServerProvider, ServerProviderVersionAdvisory } from "@trumbo-code/contracts";
...
export function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      ...
      detail:
        provider.message ??
        "This provider is installed but disabled for new sessions in Trumbo Code.",
    };
  }
