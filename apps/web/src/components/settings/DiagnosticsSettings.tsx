  ...
  return (
    ...
    >
      <table className="w-full min-w-[1040px] table-fixed text-left text-xs">
        ...
        <thead className="sticky top-0 z-10 border-b border-border/60 bg-muted/30 text-[11px] uppercase tracking-[0.08em] text-muted-foreground/70">
          <tr>
        ...
        if (!result.value.signaled) {
          ...
          if (isStaleProcessSignalMessage(message)) {
            toastManager.add({
              ...
              description:
                "The process is not a child of the Trumbo Server. It might already have exited.",
            });
            return;
          }
      ...
      >
        <StatsGrid>
          <StatBlock
            ...
            tooltip="Approximate active CPU time for the Trumbo server root process and its descendants during the selected window. It grows only while sampled processes use CPU and older samples leave as the window moves."
          />
          <StatBlock
            label="Samples"
