  ...
  return (
    <Tooltip>
      ...
      <TooltipPopup side="top" className="max-w-72 whitespace-normal leading-relaxed">
        Trumbo Code does not recognize this condition yet. It can still be saved, but it may not
...
function WhenExpressionNodeEditor({
  ...
  if (node.type === "not") {
    return (
      <div
        className={cn(
          "space-y-2 rounded-md border border-border/60 bg-muted/15 p-2",
  ...
  return (
    <div
      className={cn(
        "space-y-2 rounded-md border border-border/55 bg-muted/10 p-2",
  ...
  return (
    <div className="w-[min(34rem,calc(100vw-2rem))] space-y-3">
      ...
      <div className="relative">
        ...
        {parseError ? (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center rounded-md border border-destructive/30 bg-background/75 p-4 text-center text-xs text-destructive backdrop-blur-[1px]">
      ...
      >
        {!isElectron ? (
          <div className="flex items-start gap-2 border-b border-warning/20 bg-warning/5 px-3 py-2.5 text-[12px] leading-relaxed text-muted-foreground sm:px-4">
            ...
            <p>
              Some shortcuts may be claimed by the browser before Trumbo Code sees them. Use the
