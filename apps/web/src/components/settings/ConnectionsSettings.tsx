...
import { connectionStatusText } from "@trumbo-code/client-runtime/connection";
import {
          ...
          <Dialog open={isRevealDialogOpen} onOpenChange={setIsRevealDialogOpen}>
            ...
            <DialogPopup className="max-w-md">
              ...
              <DialogPanel className="space-y-4">
                ...
                />
                {shareablePairingUrl ? (
                  <div className="flex justify-center rounded-md border border-border/60 bg-muted/20 p-4">
                    <QRCodeSvg
                      value={shareablePairingUrl}
                      size={132}
                      level="M"
...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "Trumbo Connect" : null,
  ].filter((value): value is string => value !== null);
...
function CloudLinkSwitch({
  ...
  disabledReason,
  onCheckedChange,
  ariaLabel = "Enable Trumbo Connect",
}: {
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Trumbo Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Trumbo Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      toastManager.add({
        type: "success",
        title: enabled
          ? "Trumbo Connect linked"
          : publishAgentActivity
            ? "Trumbo Connect tunnel disabled"
            : "Trumbo Connect unlinked",
        description: enabled
          ? "This environment is available through Trumbo Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Trumbo Connect.",
      });
    }
    setIsUpdating(false);
  };
  ...
  return (
    <>
      <SettingsRow
        title="Trumbo Connect"
        description={
          managedTunnelActive
            ? "This environment is available to your other devices through Trumbo Connect."
            : "Make this environment available to your other devices through Trumbo Connect."
        }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a Trumbo Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click “Add environment” to pair another environment, or connect one from Trumbo Connect."
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Trumbo Code restarts when you change this."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Trumbo Code will restart to expose this environment over the network."
                    : "Trumbo Code will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Trumbo Code will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Trumbo Code until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Trumbo Code will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Trumbo Code will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Trumbo Code will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Trumbo Code will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Trumbo Code will restart the local backend with Tailscale Serve enabled and ask
