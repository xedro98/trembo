...
export function resolveThreadRowClassName(input: {
  ...
  const baseClassName =
    "h-7 w-full translate-x-0 cursor-pointer justify-start px-2 text-left font-stat text-sm font-medium leading-tight select-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-ring sm:h-7";

  if (input.isSelected && input.isActive) {
    return cn(
      baseClassName,
      "bg-primary/15 text-brand font-medium hover:bg-primary/20 hover:text-brand dark:bg-primary/22 dark:hover:bg-primary/28",
    );
  }
  ...
  if (input.isSelected) {
    return cn(
      baseClassName,
      "bg-primary/8 text-foreground hover:bg-primary/12 hover:text-foreground dark:bg-primary/15 dark:hover:bg-primary/20",
    );
  }

  if (input.isActive) {
    return cn(baseClassName, "text-brand font-medium hover:text-brand");
  }

  return cn(baseClassName, "text-muted-foreground transition-colors hover:text-brand");
}
