...
describe("getSidebarThreadIdsToPrewarm", () => {
  it("returns only the first visible thread ids up to the prewarm limit", () => {
    expect(getSidebarThreadIdsToPrewarm(["t1", "t2", "trumbo-code"], 2)).toEqual(["t1", "t2"]);
  });
    ...
    expect(
      resolveSidebarNewThreadSeedContext({
        ...
        activeThread: {
          ...
          worktreePath: "/repo/.trumbo-code/worktrees/existing",
        },
        activeDraftThread: {
          projectId: "project-1",
          branch: "feature/draft",
          worktreePath: "/repo/.trumbo-code/worktrees/draft",
          envMode: "worktree",
...
describe("resolveThreadRowClassName", () => {
  it("uses the brand palette when a thread is both selected and active", () => {
  ...
  it("uses selected hover colors for selected threads", () => {
    ...
    expect(className).toContain("bg-primary/8");
    expect(className).toContain("hover:bg-primary/12");
    expect(className).not.toContain("hover:bg-accent");
  });

  it("uses brand-green text for active-only threads", () => {
  ...
  it("uses muted-foreground text for inactive threads with brand hover", () => {
