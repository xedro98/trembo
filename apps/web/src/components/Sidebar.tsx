...
} from "@trumbo-code/contracts/settings";
...
import { TrumboSidebarAccount } from "./trumbo-auth/TrumboConnectSidebarSignIn";
import { TrumboSidebarHeaderAuth } from "./trumbo-auth/TrumboSidebarHeaderAuth";
import { SidebarEmptyProjectsCallout } from "./desktop/SidebarEmptyProjectsCallout";
    ...
    >
      {shouldShowThreadPanel && showEmptyThreadState ? (
        <SidebarMenuSubItem className="w-full" data-thread-selection-safe>
          <div data-thread-selection-safe className="w-full translate-x-0 px-2 py-2 text-left">
            <p className="text-[10px] text-muted-foreground/60">No threads yet</p>
            {onCreateThread ? (
              <div className="mt-2">
                <Button
                  type="button"
    ...
    <>
      <div className="group/project-header relative">
        <SidebarMenuButton
          ...
          size="sm"
          className={`gap-2 px-2 py-1.5 pr-8 text-left font-stat text-sm font-medium leading-tight text-muted-foreground transition-colors hover:text-brand group-hover/project-header:text-foreground max-sm:pr-14 ${
...
const SidebarChromeHeader = memo(function SidebarChromeHeader({
  ...
  return (
    <SidebarHeader
      className={cn(
        "@container/sidebar-header h-[var(--workspace-topbar-height)] shrink-0 flex-row items-center gap-2 px-3 py-0 md:px-3",
        isElectron && "drag-region",
      )}
    >
      <SidebarTrigger className="md:hidden" />
      <SidebarBrand />
      <TrumboSidebarHeaderAuth className="ml-auto" />
    </SidebarHeader>
  );
});
...
function SidebarBrand() {
  ...
  return (
    <Link
      ...
      className={cn(
        "sidebar-brand ml-[var(--workspace-titlebar-content-left)] flex h-7 w-fit min-w-0 shrink-0 items-center gap-1.5 overflow-hidden rounded-md text-foreground outline-hidden ring-ring focus-visible:ring-2",
      )}
      to="/"
    >
      <TrumboWordmark className="size-4 shrink-0 text-[#2BBF77]" />
      <span className="font-heading truncate text-sm font-semibold tracking-tight">Trumbo</span>
      {stageLabel ? (
        <span className="sidebar-brand-stage shrink-0 items-center whitespace-nowrap text-[9px] font-medium uppercase tracking-[0.18em] text-muted-foreground/50">
          {stageLabel}
        </span>
      ) : null}
    </Link>
...
const SidebarChromeFooter = memo(function SidebarChromeFooter() {
  const { isMobile, setOpenMobile } = useSidebar();
  const showDesktopAccount = isElectron || isNativeTrumboDesktop();
  ...
  return (
    <SidebarFooter className="mt-auto border-t border-t-dotted border-grid-line p-3">
      {showDesktopAccount ? (
        <TrumboSidebarAccount />
      ) : (
        <>
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                size="sm"
                className="gap-2 px-2 py-1.5 font-stat text-sm font-medium leading-none text-muted-foreground transition-colors hover:text-brand"
...
const SidebarProjectsContent = memo(function SidebarProjectsContent(
  ...
  return (
    <SidebarContent className="gap-0 px-2 pt-2">
      <SidebarGroup className="px-1 pt-1 pb-2">
        <SidebarMenu>
          <SidebarMenuItem>
            <CommandDialogTrigger
              render={
                <SidebarMenuButton
                  size="sm"
                  className="gap-2 px-2 py-2 font-stat text-sm font-medium leading-tight text-muted-foreground transition-colors hover:text-brand focus-visible:ring-0"
      ...
      <SidebarGroup className="px-2 py-2">
        ...
        {projectsLength === 0 ? (
          <SidebarEmptyProjectsCallout onAddProject={openAddProject} />
        ) : null}
      </SidebarGroup>
    </SidebarContent>
  );
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      <div className="mt-2 border-t border-t-dotted border-grid-line" />
      <SidebarChromeFooter />
    </>
  );
}
