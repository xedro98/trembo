...
} from "@trumbo-code/client-runtime/platform";
import {
...
import { bootstrapRemoteBearerSession } from "@trumbo-code/client-runtime/authorization";
import { fetchRemoteEnvironmentDescriptor } from "@trumbo-code/client-runtime/environment";
import {
  managedRelayAccountChanges,
  managedRelaySessionAtom,
} from "@trumbo-code/client-runtime/relay";
import { EnvironmentRpcRequestObserver } from "@trumbo-code/client-runtime/rpc";
import {
...
function clientMetadata() {
  ...
  return {
    label: desktop ? "Trumbo Code Desktop" : "Trumbo Code Web",
    ...
    const cloudSession = CloudSession.of({
      clerkToken: Effect.gen(function* () {
        ...
        if (session === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "Sign in to Trumbo Connect to connect this environment.",
          });
        }
        ...
        if (token === null) {
          return yield* new ConnectionBlockedError({
            reason: "authentication",
            detail: "The Trumbo Connect session is unavailable.",
          });
        }
        return token;
      }),
