import { ProjectId, ThreadId } from "@trumbo-code/contracts";
...
import {
  ...
  type UiState,
  assignThreadTabToGroup,
  createThreadTabGroup,
  closeThreadTab,
  openThreadTab,
} from "./uiStateStore";
...
describe("thread tab groups", () => {
  it("creates a group and assigns tabs", () => {
    const opened = openThreadTab(makeUiState(), "env:thread-a");
    const withSecond = openThreadTab(opened, "env:thread-b");
    const created = createThreadTabGroup(withSecond, { name: "Backend", colorId: "blue" });
    const groupId = created.threadTabGroups[0]!.id;

    expect(created.threadTabGroups).toEqual([
      expect.objectContaining({ id: groupId, name: "Backend", colorId: "blue" }),
    ]);

    const assigned = assignThreadTabToGroup(created, "env:thread-a", groupId);
    expect(assigned.threadTabGroupByKey["env:thread-a"]).toBe(groupId);

    const closed = closeThreadTab(assigned, "env:thread-a");
    expect(closed.threadTabGroupByKey["env:thread-a"]).toBeUndefined();
  });
});
