import { DEFAULT_CLIENT_SETTINGS } from "@trumbo-code/contracts";
...
describe("clientPersistenceStorage", () => {
  ...
  it("reports structured decode failures while preserving the fallback", async () => {
    ...
    testWindow.localStorage.setItem("trumbo-code:client-settings:v1", "not-json");
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        operation: "decode",
        storageKey: "trumbo-code:client-settings:v1",
  ...
  it("defaults word wrap on and discards obsolete wrapping preferences", async () => {
    ...
    testWindow.localStorage.setItem(
      "trumbo-code:client-settings:v1",
      JSON.stringify({
        chatWordWrap: false,
        diffWordWrap: false,
      }),
