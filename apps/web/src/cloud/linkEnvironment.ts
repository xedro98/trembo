...
import { EnvironmentRegistry } from "@trumbo-code/client-runtime/connection";
import { request, runStream } from "@trumbo-code/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@trumbo-code/client-runtime/rpc";
import { ManagedRelay } from "@trumbo-code/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Trumbo Code cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Trumbo Code cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "TRUMBO_CODE_RELAY_URL is not configured.",
      });
    }
...
// independently of Trumbo Connect while clients reach the environment out of band.
