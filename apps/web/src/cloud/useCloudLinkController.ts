...
import { findErrorTraceId } from "@trumbo-code/client-runtime/errors";
import {
...
/**
 * Drives the primary environment's Trumbo Connect link. Trumbo Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Trumbo Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Trumbo Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Trumbo Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Trumbo Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Trumbo Connect before enabling this."));
        return false;
      }
