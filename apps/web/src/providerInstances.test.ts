...
describe("isProviderInstanceConfiguredForSetup", () => {
  it("accepts warning providers that are installed and enabled", () => {
    const [entry] = deriveProviderInstanceEntries([
      {
        ...provider({ provider: ProviderDriverKind.make("trumbo"), instanceId: "trumbo" }),
        status: "warning",
        auth: { status: "unauthenticated" },
      },
    ]);

    expect(entry && isProviderInstanceConfiguredForSetup(entry)).toBe(true);
    expect(entry && isProviderInstancePickerReady(entry)).toBe(false);
  });
});
