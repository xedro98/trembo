...
import { type DeepPartial, deepMerge } from "@trumbo-code/shared/Struct";
import { fromJsonStringPretty, fromLenientJson } from "@trumbo-code/shared/schemaJson";
...
>()("trumbo-code/serverSettings/ServerSettingsService") {
...
/**
 * Native Trumbo desktop builds expose only the Trumbo provider. Force legacy
 * provider toggles off at read time so persisted multi-provider settings do
 * not leak into the desktop UI.
 */
function normalizeNativeDesktopSettings(settings: ServerSettings): ServerSettings {
  if (!isNativeTrumboDesktopBuild()) {
    return settings;
  }

  const trumboDriver = ProviderDriverKind.make("trumbo");
  const trumboInstanceId = ProviderInstanceId.make(trumboDriver);

  return {
    ...settings,
    providers: {
      ...settings.providers,
      trumbo: { ...settings.providers.trumbo, enabled: true },
    ...
    textGenerationModelSelection: {
      instanceId: trumboInstanceId,
      model:
        DEFAULT_GIT_TEXT_GENERATION_MODEL_BY_PROVIDER[trumboDriver] ??
        DEFAULT_MODEL_BY_PROVIDER[trumboDriver] ??
        "quartz-1.0-lite",
    },
  };
}

function finalizeServerSettings(settings: ServerSettings): ServerSettings {
  return resolveTextGenerationProvider(normalizeNativeDesktopSettings(settings));
}
...
const make = Effect.gen(function* () {
  ...
  return {
    ...
    getSettings: getSettingsFromCache.pipe(
      ...
      Effect.map(finalizeServerSettings),
    ),
    updateSettings: (patch) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          return finalizeServerSettings(materialized);
        }),
      ),
    get streamChanges() {
      return Stream.fromPubSub(changesPubSub).pipe(
        ...
        Stream.map(finalizeServerSettings),
      );
    },
  } satisfies ServerSettingsService["Service"];
});
