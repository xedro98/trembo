...
import { EnvironmentOrchestrationHttpApi } from "@trumbo-code/contracts";
...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["trumbo-code", "connect"]);
      assert.include(error.errors[0]?.message ?? "", "missing Trumbo Connect public configuration");

      const output = (yield* TestConsole.errorLines).join("\n");
      assert.include(output, "ERROR");
      assert.include(output, "missing Trumbo Connect public configuration");
    }).pipe(Effect.provide(Layer.mergeAll(CliRuntimeLayer, TestConsole.layer))),
  );
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Trumbo Connect\n  Exposure: disabled");
      assert.include(output, "  Authorization: missing");
      assert.include(output, "  Environment link: not provisioned");
      assert.include(output, "Next: Run `t3 connect link` to authorize and enable Trumbo Connect.");
    }),
  );
  ...
  it.effect("logs in to headless connect without enabling access", () =>
    Effect.gen(function* () {
      ...
      assert.equal(login.output, "Signed in to Trumbo Connect.");
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      ...
      assert.equal(output, "Trumbo Connect is disabled locally.");
    }),
  );
  ...
  it.effect("logs out of headless connect and removes the stored CLI authorization", () =>
    Effect.gen(function* () {
      ...
      assert.equal(output, "Signed out of Trumbo Connect locally.");
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["trumbo-code", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["trumbo-code", "project", "add"]);
