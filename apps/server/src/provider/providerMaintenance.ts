...
import { resolveCommandPath } from "@trumbo-code/shared/shell";
...
export const ProviderVersionCache = Context.Reference<Map<string, ProviderVersionCacheEntry>>(
  "trumbo-code/providerMaintenance/ProviderVersionCache",
  {
...
export const resolveProviderMaintenanceCapabilitiesEffect = Effect.fn(
  ...
  if (options?.resolvedCommandPath) {
    return resolver.resolve({
      ...options,
      env: options.env ?? (yield* readCommandLookupEnv),
      resolvedCommandPath: options.resolvedCommandPath,
    });
  }
