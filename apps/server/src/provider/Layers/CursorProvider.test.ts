...
import type { CursorSettings } from "@trumbo-code/contracts";
...
import { makeMockCliWrapper } from "../../test/mockCliWrapper.ts";
import { writeNodeCliExecutable } from "../../test/fakeCliBinary.ts";
import { assertAcpChildProcessStopped, isWindowsHost } from "../../test/testPlatform.ts";
...
const makeMockAgentWrapper = Effect.fn("makeMockAgentWrapper")(function* (
  ...
  const wrapperPath = yield* Effect.gen(function* () {
    if (isWindowsHost) {
      const winWrapperPath = path.join(dir, "fake-agent.cmd");
      const envLines = Object.entries(extraEnv ?? {}).map(
        ([key, value]) => `set "${key}=${value}"`,
      );
      const script = ["@echo off", ...envLines, `"node" "${mockAgentPath}" %*`].join("\r\n");
      yield* fileSystem.writeFileString(winWrapperPath, script);
      return winWrapperPath;
    }
    return yield* Effect.promise(() =>
      makeMockCliWrapper({
        ...
        name: "fake-agent",
        command: "node",
        args: [mockAgentPath] as readonly string[],
        ...(extraEnv ? { extraEnv } : {}),
      }),
    );
  });
...
const cursorAcpDiscoveryFailedMessage = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart Trumbo Code, and try again.",
...
const cursorCliCommandMissingMessage = [
  ...
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart Trumbo Code.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
...
describe("discoverCursorModelsViaAcp", () => {
  ...
  it("closes the ACP probe runtime after discovery completes", async () => {
    const { exitLogPath, pidLogPath, wrapperPath } = await runNode(
    ...
    await assertAcpChildProcessStopped({ exitLogPath, pidLogPath });
  });
});
