...
import { GrokSettings } from "@trumbo-code/contracts";

import { buildInitialGrokProviderSnapshot, checkGrokProviderStatus } from "./GrokProvider.ts";
import { writeFakeVersionCliExecutable } from "../../test/fakeCliBinary.ts";
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "trumbo-code-grok-version-" });
          const grokPath = yield* Effect.promise(() =>
            writeFakeVersionCliExecutable({
              dir,
              name: "grok",
              stderr: secretStderr,
              exitCode: 2,
  ...
  it.effect("reports an error when ACP model discovery is unavailable", () =>
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "trumbo-code-grok-success-" });
          const grokPath = yield* Effect.promise(() =>
            writeFakeVersionCliExecutable({
              dir,
              name: "grok",
              stdout: "grok-cli 0.0.99\n",
              exitCode: 0,
            }),
          );
