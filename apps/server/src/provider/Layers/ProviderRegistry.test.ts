...
import { deepMerge } from "@trumbo-code/shared/Struct";
import { createModelCapabilities } from "@trumbo-code/shared/model";
import { applyServerSettingsPatch } from "@trumbo-code/shared/serverSettings";
...
import {
  NonWindowsPlatform,
  normalizeTestSpawnArgs,
  spawnArgsIncludeAuthStatus,
  spawnArgsIncludeVersion,
} from "../../test/testPlatform.ts";
...
process.env.TRUMBO_CODE_CURSOR_ENABLED = "1";
...
function mockSpawnerLayer(
  ...
  return Layer.succeed(
    ...
    ChildProcessSpawner.make((command) => {
      ...
      return Effect.succeed(mockHandle(handler(normalizeTestSpawnArgs(cmd.args))));
    }),
  );
}
...
function recordingMockSpawnerLayer(
  ...
  const layer = Layer.succeed(
    ...
    ChildProcessSpawner.make((command) => {
      ...
      const normalizedArgs = normalizeTestSpawnArgs(cmd.args);
      commands.push({ args: normalizedArgs, env: cmd.options?.env });
      return Effect.succeed(mockHandle(handler(normalizedArgs)));
    }),
  );
...
function mockCommandSpawnerLayer(
  ...
  return Layer.succeed(
    ...
    ChildProcessSpawner.make((command) => {
      ...
      return Effect.succeed(mockHandle(handler(cmd.command, normalizeTestSpawnArgs(cmd.args))));
    }),
  );
}
...
const TestTrumboPlatformTokenManagerLive = Layer.succeed(
  TrumboPlatformTokenManager.TrumboPlatformTokenManager,
  TrumboPlatformTokenManager.TrumboPlatformTokenManager.of({
...
it.layer(
  Layer.mergeAll(
    ...
    TestHttpClientLive,
    NonWindowsPlatform,
    TestTrumboPlatformTokenManagerLive,
  ),
)("ProviderRegistry", (it) => {
  ...
  describe("ProviderRegistryLive", () => {
        ...
        const serverSettings = yield* makeMutableServerSettingsService(
          decodeServerSettings(
            deepMerge(encodedDefaultServerSettings, {
              ...
              providerInstances: {
                // Matches the shape the user had in `.trumbo-code/dev/settings.json`
      ...
      Effect.gen(function* () {
        ...
        const providerRegistryLayer = ProviderRegistryLive.pipe(
          ...
          Layer.provideMerge(
            mockCommandSpawnerLayer((command, args) => {
              ...
              if (spawnArgsIncludeVersion(args)) {
                return {
              ...
              if (spawnArgsIncludeAuthStatus(args)) {
                return {
              ...
              throw new Error(
                `Unexpected args: ${command} ${normalizeTestSpawnArgs(args).join(" ")}`,
              );
            }),
          ),
        );
        ...
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(
            ...
            "Cursor is disabled in Trumbo Code settings.",
          );
    ...
    it.effect("skips codex probes entirely when the provider is disabled", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(status.installed, false);
        assert.strictEqual(status.message, "Codex is disabled in Trumbo Code settings.");
      }),
    );
  });
  ...
  describe("checkClaudeProviderStatus", () => {
      ...
      Effect.gen(function* () {
        ...
        Effect.provide(
          mockSpawnerLayer((args) => {
            if (spawnArgsIncludeVersion(args)) return { stdout: "1.0.0\n", stderr: "", code: 0 };
            if (spawnArgsIncludeAuthStatus(args))
              return {
            ...
            throw new Error(`Unexpected args: ${normalizeTestSpawnArgs(args).join(" ")}`);
          }),
        ),
      ),
    );
    ...
    it.effect("includes Claude Fable 5 on supported Claude Code versions", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          mockSpawnerLayer((args) => {
            if (spawnArgsIncludeVersion(args)) return { stdout: "2.1.169\n", stderr: "", code: 0 };
            if (spawnArgsIncludeAuthStatus(args))
              return {
                stdout: '{"loggedIn":true,"authMethod":"claude.ai"}\n',
                stderr: "",
    ...
    it.effect("hides Claude Fable 5 on older Claude Code versions", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          mockSpawnerLayer((args) => {
            if (spawnArgsIncludeVersion(args)) return { stdout: "2.1.168\n", stderr: "", code: 0 };
            if (spawnArgsIncludeAuthStatus(args))
              return {
                stdout: '{"loggedIn":true,"authMethod":"claude.ai"}\n',
                stderr: "",
    ...
    it.effect("hides Claude Opus 4.7 on older Claude Code versions", () =>
      Effect.gen(function* () {
        ...
        Effect.provide(
          mockSpawnerLayer((args) => {
            if (spawnArgsIncludeVersion(args)) return { stdout: "2.1.110\n", stderr: "", code: 0 };
            if (spawnArgsIncludeAuthStatus(args))
              return {
                stdout: '{"loggedIn":true,"authMethod":"claude.ai"}\n',
                stderr: "",
    ...
    it.effect("runs Claude status probes with the configured CLAUDE_CONFIG_DIR", () => {
      const claudeConfigDir = NodePath.join(NodeOS.tmpdir(), "trumbo-code-claude-home");
      ...
      return Effect.gen(function* () {
        ...
        assert.strictEqual(status.status, "ready");
        assert.deepStrictEqual(
          recorded.commands.map((command) => command.env?.CLAUDE_CONFIG_DIR?.replaceAll("\\", "/")),
          [claudeConfigDir.replaceAll("\\", "/")],
        );
