...
import { ProviderDriverKind } from "@trumbo-code/contracts";
import type * as EffectAcpSchema from "effect-acp/schema";
import { causeErrorTag } from "@trumbo-code/shared/observability";
    ...
    if (!cursorSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Cursor is disabled in Trumbo Code settings.",
        },
      });
    }
  ...
  Effect.gen(function* () {
    ...
    const acpContext = yield* Layer.build(
      AcpSessionRuntime.layer({
        ...
        cwd: process.cwd(),
        clientInfo: { name: "trumbo-code-provider-probe", version: "0.0.0" },
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart Trumbo Code.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}
