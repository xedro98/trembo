...
import {
  assertAcpChildProcessStopped,
  assertAcpExitLogStopCount,
  isWindowsHost,
  normalizeArgvLogTokens,
...
class CursorAdapter extends Context.Service<CursorAdapter, CursorAdapterShape>()(
  "trumbo-code/provider/Layers/CursorAdapter.test/CursorAdapter",
) {}
...
async function makeMockAgentWrapper(
  ...
  if (options?.initialDelaySeconds) {
    await new Promise((resolve) => setTimeout(resolve, (options.initialDelaySeconds ?? 0) * 1000));
  }
  if (isWindowsHost) {
    const wrapperPath = NodePath.join(dir, "fake-agent.cmd");
    const envLines = Object.entries(extraEnv ?? {}).map(([key, value]) => `set "${key}=${value}"`);
    const script = ["@echo off", ...envLines, `"${mockAgentCommand}" "${mockAgentPath}" %*`].join(
      "\r\n",
    );
    await NodeFSP.writeFile(wrapperPath, script, "utf8");
    return wrapperPath;
  }
  return makeMockCliWrapper({
    dir,
    name: "fake-agent",
    command: mockAgentCommand,
    args: [...mockAgentArgs] as readonly string[],
    ...(extraEnv ? { extraEnv } : {}),
  });
}
...
async function readArgvLog(filePath: string) {
  ...
  return raw
    ...
    .map((line) => normalizeArgvLogTokens(line.split("\t").filter((token) => token.length > 0)));
}
  ...
  Layer.effect(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "trumbo-code-cursor-adapter-test-",
      }),
    ),
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect("closes the ACP child process when a session stops", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => assertAcpChildProcessStopped({ exitLogPath, pidLogPath }));
    }),
  );
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          isWindowsHost
            ...
            : assertAcpExitLogStopCount(exitLogPath, 2),
        );
      }),
  );
  ...
  it.effect(
    ...
    () => {
      ...
      const customAdapterLayer = Layer.effect(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "trumbo-code-cursor-adapter-custom-instance-",
          }),
        ),
        Layer.provideMerge(NodeServices.layer),
      );
