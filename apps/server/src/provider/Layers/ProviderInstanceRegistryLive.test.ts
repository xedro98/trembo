...
import { makeClaudeContinuationGroupKey } from "../Drivers/ClaudeHome.ts";
...
describe("ProviderInstanceRegistryLive — multi-instance codex slice", () => {
  ...
  it.live("boots two independent codex instances from a ProviderInstanceConfigMap", () =>
    Effect.gen(function* () {
      ...
      const personalLayout = yield* resolveCodexHomeLayout(
        makeCodexConfig({ homePath: "/home/julius/.codex_personal" }),
      );
      expect(personalSnapshot.continuation?.groupKey).toBe(personalLayout.continuationKey);
      ...
      const workLayout = yield* resolveCodexHomeLayout(
        makeCodexConfig({ homePath: "/home/julius/.codex" }),
      );
      expect(workSnapshot.continuation?.groupKey).toBe(workLayout.continuationKey);
...
describe("ProviderInstanceRegistryLive — all drivers slice", () => {
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      expect(codexSnapshot.continuation?.groupKey).toBe(codexLayout.continuationKey);
      ...
      expect(claudeSnapshot.continuation?.groupKey).toBe(
        yield* makeClaudeContinuationGroupKey({ homePath: "/home/julius/.claude-work" }),
      );
