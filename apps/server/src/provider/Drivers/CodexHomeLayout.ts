...
import { ProviderDriverKind, type CodexSettings } from "@trumbo-code/contracts";
...
function continuationKeyForHomePath(homePath: string): string {
  return `codex:home:${homePath.replaceAll("\\", "/")}`;
}
...
export const resolveCodexHomeLayout = Effect.fn("resolveCodexHomeLayout")(function* (
  ...
  if (shadowHomePath.length === 0) {
    return {
      ...
      continuationKey: continuationKeyForHomePath(sharedHomePath),
    };
  }
