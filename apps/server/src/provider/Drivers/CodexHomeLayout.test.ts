...
import { CodexSettings } from "@trumbo-code/contracts";
import { normalizeTestPath } from "../../test/testPlatform.ts";
import {
...
it.layer(NodeServices.layer)("CodexHomeLayout", (it) => {
  describe("resolveCodexHomeLayout", () => {
    it.effect("uses direct CODEX_HOME when no shadow home is configured", () =>
      Effect.gen(function* () {
        const homePath = yield* makeTempDir("trumbo-code-codex-home-");
        ...
        expect(layout).toMatchObject({
          ...
          continuationKey: `codex:home:${normalizeTestPath(homePath)}`,
        });
      }),
    );
    ...
    it.effect("uses the shared home for continuation and the shadow home for runtime", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("trumbo-code-codex-shared-");
        const shadowRoot = yield* makeTempDir("trumbo-code-codex-shadow-root-");
        ...
        expect(layout).toMatchObject({
          ...
          continuationKey: `codex:home:${normalizeTestPath(sharedHome)}`,
        });
      }),
    );
  });
  ...
  describe("materializeCodexShadowHome", () => {
    ...
    it.effect("preserves filesystem operation, paths, and cause", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const sharedRoot = yield* makeTempDir("trumbo-code-codex-shared-root-");
