...
import { dedupeRemoteBranchesWithLocalMatches } from "@trumbo-code/shared/git";
import { compactTraceAttributes } from "@trumbo-code/shared/observability";
import { decodeJsonResult } from "@trumbo-code/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `trumbo-code-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
...
export const makeGitVcsDriverCore = Effect.fn("makeGitVcsDriverCore")(function* () {
  ...
  const readStatusDetailsRemote = Effect.fn("readStatusDetailsRemote")(function* (cwd: string) {
    ...
    if (branchResult.exitCode !== 0) {
      ...
      // Gracefully degrade for any branch lookup failure (detached HEAD,
      // corrupt repo, no branches, etc.) instead of erroring and generating
      // noise in the observability dashboard.
      return NON_REPOSITORY_REMOTE_STATUS_DETAILS;
    }
