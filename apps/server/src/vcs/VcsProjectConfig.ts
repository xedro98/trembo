...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@trumbo-code/contracts";
import { fromLenientJson } from "@trumbo-code/shared/schemaJson";
...
>()("trumbo-code/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".trumbo-code", "vcs.json");
