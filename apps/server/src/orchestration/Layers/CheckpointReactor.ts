...
const make = Effect.gen(function* () {
  ...
  const handleRevertRequested = Effect.fn("handleRevertRequested")(function* (
    ...
    // Resolve the workspace CWD for the revert. Try the active provider session
    // first, then fall back to the thread/project config — the revert is a git
    // operation that doesn't need an active provider session, just the cwd.
    const sessionRuntime = yield* resolveSessionRuntimeForThread(event.payload.threadId);
    const threadProjects = yield* resolveThreadProjects(thread.projectId);
    const revertCwd = Option.match(sessionRuntime, {
      onNone: () =>
        resolveThreadWorkspaceCwd({
          thread: { projectId: thread.projectId, worktreePath: thread.worktreePath ?? null },
    ...
    if (!revertCwd) {
      yield* appendRevertFailureActivity({
        threadId: event.payload.threadId,
        turnCount: event.payload.turnCount,
        detail: "Could not determine the workspace directory for this thread.",
        createdAt: now,
      }).pipe(Effect.catch(() => Effect.void));
      return;
    }
