...
describe("OrchestrationEngine", () => {
  ...
  it("does not regress a generated branch to a stale temporary worktree branch", async () => {
    ...
    await system.run(
      engine.dispatch({
        ...
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
        branch: "trumbo-code/generated-branch-name",
    ...
    await system.run(
      engine.dispatch({
        ...
        threadId: ThreadId.make("thread-branch-race"),
        branch: "trumbo-code/1234abcd",
        expectedBranch: "trumbo-code/1234abcd",
      }),
    );

    const snapshot = await system.readModel();
    expect(snapshot.threads[0]?.branch).toBe("trumbo-code/generated-branch-name");
    await system.dispose();
  });
  ...
  it("allows authoritative worktree bootstrap to assign a temporary branch", async () => {
    ...
    const snapshot = await system.readModel();
    expect(snapshot.threads[0]?.branch).toBe("trumbo-code/1234abcd");
