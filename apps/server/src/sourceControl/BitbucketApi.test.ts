...
import { GitCommandError } from "@trumbo-code/contracts";
...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/xedro98/trumbo-code/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/trumbo-code",
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "xedro98/trumbo-code",
      workspace: { slug: "xedro98" },
    },
  },
};
...
const repositoryJson = {
  full_name: "xedro98/trumbo-code",
  links: {
    html: { href: "https://bitbucket.org/xedro98/trumbo-code" },
    clone: [
      { name: "https", href: "https://bitbucket.org/xedro98/trumbo-code.git" },
      { name: "ssh", href: "git@bitbucket.org:xedro98/trumbo-code.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriver["Service"]["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:xedro98/trumbo-code.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:xedro98/trumbo-code.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            TRUMBO_CODE_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            TRUMBO_CODE_BITBUCKET_EMAIL: "user@example.com",
            TRUMBO_CODE_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/xedro98/trumbo-code/pull-requests/42",
      baseRefName: "main",
      headRefName: "feature/source-control",
      state: "open",
      updatedAt: Option.some(DateTime.makeUnsafe("2026-01-02T00:00:00.000Z")),
      isCrossRepository: true,
      headRepositoryNameWithOwner: "octocat/trumbo-code",
      headRepositoryOwnerLogin: "octocat",
    });
    assert.strictEqual(
      execute.mock.calls[0]?.[0].url,
      "https://api.test.local/2.0/repositories/xedro98/trumbo-code/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "xedro98/trumbo-code" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/xedro98/trumbo-code/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "xedro98/trumbo-code",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "xedro98/trumbo-code",
      url: "https://bitbucket.org/xedro98/trumbo-code.git",
      sshUrl: "git@bitbucket.org:xedro98/trumbo-code.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/xedro98/trumbo-code",
          "https://api.test.local/2.0/repositories/xedro98/trumbo-code/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/xedro98/trumbo-code");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/trumbo-code" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:xedro98/trumbo-code.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/trumbo-code")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/trumbo-code",
          links: {
            html: { href: "https://bitbucket.org/octocat/trumbo-code" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/trumbo-code.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/trumbo-code.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/trumbo-code.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "trumbo-code/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "trumbo-code/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "trumbo-code/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
