...
import { VcsRepositoryDetectionError } from "@trumbo-code/contracts";
...
it.effect("routes GitHub remotes to the GitHub provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:xedro98/trumbo-code.git" }],
    });
...
it.effect("routes Bitbucket remotes to the Bitbucket provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@bitbucket.org:xedro98/trumbo-code.git" }],
    });
