...
import { sanitizeBranchFragment } from "@trumbo-code/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@trumbo-code/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("TRUMBO_CODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("TRUMBO_CODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("TRUMBO_CODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("TRUMBO_CODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("trumbo-code/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `trumbo-code/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set TRUMBO_CODE_BITBUCKET_EMAIL and TRUMBO_CODE_BITBUCKET_API_TOKEN, or TRUMBO_CODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
