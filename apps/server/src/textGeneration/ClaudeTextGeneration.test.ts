import { ClaudeSettings, ProviderInstanceId } from "@trumbo-code/contracts";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "trumbo-code-claude-text-generation-test-",
...
function makeFakeClaudeBinary(dir: string) {
  return Effect.gen(function* () {
    ...
    const claudePath = yield* Effect.promise(() =>
...
if (process.env.T3_FAKE_CLAUDE_ARGS_MUST_CONTAIN && !args.includes(process.env.T3_FAKE_CLAUDE_ARGS_MUST_CONTAIN)) {
  fail("args missing expected content", 2);
}
if (process.env.T3_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN && args.includes(process.env.T3_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN)) {
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "trumbo-code-claude-text-" });
