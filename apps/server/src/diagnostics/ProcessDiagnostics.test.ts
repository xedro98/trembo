...
import { NonWindowsPlatform } from "../test/testPlatform.ts";
  ...
  it.effect("queries processes through the ChildProcessSpawner service", () =>
    Effect.gen(function* () {
      ...
      const spawnerLayer = Layer.succeed(
        ...
        ChildProcessSpawner.make((command) => {
          ...
          return Effect.succeed(
            mockHandle({
              stdout: [
                ` ${process.pid}     1 ${process.pid} Ss 0.0 1024 01:02.03 trumbo-code server`,
      ...
      const layer = ProcessDiagnostics.layer.pipe(
        ...
        Layer.provide(NonWindowsPlatform),
      );
  ...
  it.effect("does not allow signaling the diagnostics query process", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({
        ...
        signal: "SIGINT",
        signaled: false,
        message: Option.some("Process 4242 is not a live descendant of the Trumbo server."),
      });
    }),
  );
});
