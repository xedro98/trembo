...
import { expectPathContains } from "../test/testPlatform.ts";
...
const makeTempDir = Effect.gen(function* () {
  ...
  return yield* fileSystem.makeTempDirectoryScoped({
    prefix: "trumbo-code-project-favicon-",
  });
});
  ...
  describe("resolvePath", () => {
    ...
    it.effect("resolves icon hrefs from project source files", () =>
      Effect.gen(function* () {
        ...
        expect(resolved).not.toBeNull();
        expect(expectPathContains(resolved ?? "", "public/brand/logo.svg")).toBe(true);
      }),
    );
