...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:Trumbo/trumbo-code.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/trumbo/trumbo-code");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("trumbo/trumbo-code");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("trumbo");
      expect(identity?.name).toBe("trumbo-code");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:Trumbo/trumbo-code.git"]);
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/trumbo-code.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:Trumbo/trumbo-code.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:Trumbo/platform/trumbo-code.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/trumbo/platform/trumbo-code");
      expect(identity?.displayName).toBe("trumbo/platform/trumbo-code");
      expect(identity?.owner).toBe("trumbo");
      expect(identity?.name).toBe("trumbo-code");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/trumbo/trumbo-code");
        expect(refreshedIdentity?.name).toBe("trumbo-code");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/trumbo/trumbo-code");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:Trumbo/trumbo-code-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/trumbo/trumbo-code");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/trumbo/trumbo-code-next");
      expect(refreshedIdentity?.displayName).toBe("trumbo/trumbo-code-next");
      expect(refreshedIdentity?.name).toBe("trumbo-code-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
