...
} from "@trumbo-code/shared/git";
import {
  getChangeRequestTerminologyForKind,
  type ChangeRequestTerminology,
} from "@trumbo-code/shared/sourceControl";

import { GitManagerError, GitPullRequestMaterializationError } from "@trumbo-code/contracts";
import * as TextGeneration from "../textGeneration/TextGeneration.ts";
import * as ProjectSetupScriptRunner from "../project/ProjectSetupScriptRunner.ts";
import { extractBranchNameFromRemoteRef } from "./remoteRefs.ts";
import * as ServerSettings from "../serverSettings.ts";
import type { GitManagerServiceError } from "@trumbo-code/contracts";
import * as GitVcsDriver from "../vcs/GitVcsDriver.ts";
import * as SourceControlProviderRegistry from "../sourceControl/SourceControlProviderRegistry.ts";
import type { ChangeRequest } from "@trumbo-code/contracts";
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `trumbo-code/pr-${pullRequest.number}/${suffix}`;
}
...
export const make = Effect.gen(function* () {
  ...
  const runPrStep = Effect.fn("runPrStep")(function* (
    ...
    const bodyFile = path.join(
      tempDir,
      `trumbo-code-pr-body-${process.pid}-${yield* randomUUIDv4(cwd)}.md`,
    );
