...
import { parsePersistedServerObservabilitySettings } from "@trumbo-code/shared/serverSettings";
import { DesktopBackendBootstrap, PortSchema } from "@trumbo-code/contracts";
...
export const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to TRUMBO_CODE_HOME)."),
  Flag.optional,
);
...
export const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to TRUMBO_CODE_LOG_WS_EVENTS).",
  ),
...
const EnvServerConfig = Config.all({
  logLevel: Config.logLevel("TRUMBO_CODE_LOG_LEVEL").pipe(Config.withDefault("Info")),
  traceMinLevel: Config.logLevel("TRUMBO_CODE_TRACE_MIN_LEVEL").pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.boolean("TRUMBO_CODE_TRACE_TIMING_ENABLED").pipe(
    Config.withDefault(true),
  ),
  traceFile: Config.string("TRUMBO_CODE_TRACE_FILE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  traceMaxBytes: Config.int("TRUMBO_CODE_TRACE_MAX_BYTES").pipe(
    Config.withDefault(10 * 1024 * 1024),
  ),
  traceMaxFiles: Config.int("TRUMBO_CODE_TRACE_MAX_FILES").pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.int("TRUMBO_CODE_TRACE_BATCH_WINDOW_MS").pipe(Config.withDefault(200)),
  otlpTracesUrl: Config.string("TRUMBO_CODE_OTLP_TRACES_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  otlpMetricsUrl: Config.string("TRUMBO_CODE_OTLP_METRICS_URL").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  ...
  otlpServiceName: Config.string("TRUMBO_CODE_OTLP_SERVICE_NAME").pipe(
    Config.withDefault("trumbo-code-server"),
  ),
  mode: Config.schema(ServerConfig.RuntimeMode, "TRUMBO_CODE_MODE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  port: Config.port("TRUMBO_CODE_PORT").pipe(Config.option, Config.map(Option.getOrUndefined)),
  host: Config.string("TRUMBO_CODE_HOST").pipe(Config.option, Config.map(Option.getOrUndefined)),
  trumboCodeHome: Config.string("TRUMBO_CODE_HOME").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: Config.boolean("TRUMBO_CODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("TRUMBO_CODE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("TRUMBO_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("TRUMBO_CODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServeEnabled: Config.boolean("TRUMBO_CODE_TAILSCALE_SERVE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  tailscaleServePort: Config.port("TRUMBO_CODE_TAILSCALE_SERVE_PORT").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
    ...
    const baseDir = yield* resolveBaseDir(
      Option.getOrUndefined(
        resolveOptionPrecedence(
          ...
          Option.fromUndefinedOr(env.trumboCodeHome),
          Option.fromUndefinedOr(bootstrap?.trumboCodeHome),
        ),
      ),
    );
