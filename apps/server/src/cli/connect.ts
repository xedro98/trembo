...
import { RelayOkResponse } from "@trumbo-code/contracts/relay";
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `t3 connect link` to authorize and enable Trumbo Connect."
    : !status.desired
      ? "Run `t3 connect link` to enable Trumbo Connect."
      : !status.linked
        ? "Start Trumbo to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Trumbo relay client is required for Trumbo Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const logCloudDisconnectFailure = (
  ...
  Effect.logWarning("Trumbo Connect disconnect operation failed.").pipe(
    Effect.annotateLogs({
      operation,
      clearAuthorization,
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.warn(
        "Trumbo Connect is disabled, but the running server could not stop its tunnel.\nRestart that server to stop the connector.",
      );
    } else {
      yield* Console.log("Trumbo Connect is disabled locally.");
    }
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log("Signed out of Trumbo Connect locally.");
  }
});
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Trumbo Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        const tokens = yield* CliTokenManager.CloudCliTokenManager;
        yield* tokens.get;
        yield* Console.log("Signed in to Trumbo Connect.");
      }),
    ),
  ),
);
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Trumbo Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (!flags.publishOnly) {
          ...
          if (Option.isNone(installed)) {
            yield* Console.log(
              "Trumbo Connect setup cancelled. The relay client was not installed.",
            );
            return;
          }
          yield* Console.log(
        ...
        yield* Console.log(
          flags.publishOnly
            ? "This environment will publish agent activity to your mobile clients the next time Trumbo starts (no managed tunnel)."
            : "This Trumbo environment will be available through Trumbo Connect the next time Trumbo starts.",
        );
      }),
    ),
  ),
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Trumbo Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only Trumbo Connect link.");
          }
        ...
        // out of band without Trumbo Connect.
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A Trumbo Connect link is already pending. Start Trumbo to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart Trumbo to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Trumbo Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Trumbo Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect").pipe(
  Command.withDescription("Manage headless Trumbo Connect access."),
