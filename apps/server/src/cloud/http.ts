...
import { isSecureRelayUrl } from "@trumbo-code/shared/relayUrl";
  ...
  Effect.mapError(
    () =>
      new EnvironmentHttpInternalServerError({
        message: "TRUMBO_CODE_RELAY_URL must be configured as a secure absolute HTTPS origin.",
      }),
  ),
);
...
const relayClientRequest = <A>(
  ...
  HttpClientRequest.post(input.url).pipe(
    ...
    Effect.mapError(
      (cause) =>
        new EnvironmentHttpInternalServerError({
          message: `Trumbo Connect relay request failed: ${String(cause)}`,
        }),
    ),
...
const reconcileDesiredCloudLinkWith = Effect.fn("environment.cloud.reconcileDesiredLinkWith")(
  ...
  Effect.catchIf(
    ServerSecretStore.isSecretStoreError,
    failEnvironmentCloudInternalError("Could not persist desired Trumbo Connect link state."),
  ),
  Effect.catchTags({
