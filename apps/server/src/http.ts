...
import { decodeOtlpTraceRecords } from "@trumbo-code/shared/observability";
...
const DESKTOP_RENDERER_ORIGINS = ["trumbo://app", "trumbo-dev://app"];
...
export function resolveStaticFileCacheControl(pathname: string): string | undefined {
...
export const staticAndDevRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    if (!fileInfo || fileInfo.type !== "File") {
      if (isStaticAssetRequestPath(url.value.pathname)) {
        return HttpServerResponse.text("Not Found", { status: 404 });
      }
    ...
    const cacheControl = resolveStaticFileCacheControl(url.value.pathname);
