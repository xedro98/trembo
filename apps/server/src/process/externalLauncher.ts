...
import { isCommandAvailable, resolveSpawnCommand } from "@trumbo-code/shared/shell";
...
const resolveAvailableCommand = Effect.fn("externalLauncher.resolveAvailableCommand")(function* (
  ...
  for (const command of commands) {
    // Catch CommandResolutionError — if a command isn't on PATH, it's simply
    // not available. Don't propagate the error; try the next candidate.
    const available = yield* isCommandAvailable(command, { env }).pipe(
...
const buildAvailableEditors = Effect.fn("externalLauncher.buildAvailableEditors")(function* (
  ...
  for (const editor of EDITORS) {
    if (editor.commands === null) {
      ...
      const fileManagerAvailable = yield* isCommandAvailable(command, { env }).pipe(
        Effect.orElseSucceed(() => false as boolean),
      );
...
>()("trumbo-code/process/externalLauncher") {}
