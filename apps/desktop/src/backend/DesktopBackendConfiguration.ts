...
>()("@trumbo-code/desktop/backend/DesktopBackendConfiguration") {}
...
const DESKTOP_BACKEND_ENV_NAMES = [
  "TRUMBO_CODE_PORT",
  "TRUMBO_CODE_MODE",
  "TRUMBO_CODE_NO_BROWSER",
  "TRUMBO_CODE_HOST",
  "TRUMBO_CODE_DESKTOP_WS_URL",
  "TRUMBO_CODE_DESKTOP_LAN_ACCESS",
  "TRUMBO_CODE_DESKTOP_LAN_HOST",
  "TRUMBO_CODE_DESKTOP_HTTPS_ENDPOINTS",
  "TRUMBO_CODE_TAILSCALE_SERVE",
  "TRUMBO_CODE_TAILSCALE_SERVE_PORT",
] as const;
...
const resolvePrimaryStartConfig = Effect.fn("desktop.backendConfiguration.resolvePrimary")(
  ...
  > {
    ...
    const bootstrap = {
      ...
      trumboCodeHome: environment.baseDir,
    ...
    return {
      ...
      // Primary wants process.env (PATH, dev-runner's TRUMBO_CODE_HOME, etc.).
      extendEnv: true,
      bootstrap,
...
> {
  ...
  const bootstrap = {
    ...
    noBrowser: true,
    port: input.port,
    // Omit trumboCodeHome so the Linux backend uses its own home dir instead of
  ...
  // Build an explicit copy of process.env minus TRUMBO_CODE_HOME (dev-runner
  // exports the Windows-side base dir for the primary; if it leaks into
  // the WSL backend the Linux side ends up sharing C:\Users\...\.trumbo-code via
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "TRUMBO_CODE_HOME") continue;
  ...
  const baseConfig = {
    ...
    // env is already a complete process.env minus TRUMBO_CODE_HOME; pass it
