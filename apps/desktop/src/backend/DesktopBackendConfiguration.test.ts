    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          TRUMBO_CODE_PORT: "9999",
          TRUMBO_CODE_MODE: "desktop",
          TRUMBO_CODE_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "trumbo-code-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.TRUMBO_CODE_PORT);
        assert.isUndefined(first.env.TRUMBO_CODE_MODE);
        assert.isUndefined(first.env.TRUMBO_CODE_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.trumboCodeHome, environment.baseDir);
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.bootstrap.trumboCodeHome, environment.baseDir);
          assert.isTrue(Option.isNone(config.preflightFailure));
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line trumbo-code/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    ...
    try {
      ...
      // oxlint-disable-next-line trumbo-code/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
