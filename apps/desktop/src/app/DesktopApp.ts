...
import * as DesktopTrumboAuth from "./DesktopTrumboAuth.ts";
...
export class DesktopDevelopmentBackendPortRequiredError extends Schema.TaggedErrorClass<DesktopDevelopmentBackendPortRequiredError>()(
  ...
  override get message(): string {
    return "TRUMBO_CODE_PORT is required in desktop development.";
  }
}
...
const bootstrap = Effect.gen(function* () {
  ...
  yield* electronProtocol.registerDesktopProtocol({
    ...
    clerkFrontendApiHostname: DesktopTrumboAuth.desktopTrumboPlatformHostname,
  });
  ...
  if (!(yield* Ref.get(state.quitting))) {
    if (environment.isDevelopment) {
      // Dev loads from the Vite server, which is already listening — safe to
      // open the main window immediately.
      yield* desktopWindow.ensureMain.pipe(
        Effect.catch((error) =>
          logBootstrapWarning("bootstrap failed to open main window early", {
            error: error.message,
          }),
        ),
      );
    } else {
      // Packaged builds proxy trumbo-code:// through a loopback backend that
      // cold-starts after this point. Opening the main window first leaves a
      // blank white frame until a later reload succeeds. Show the splash
      // until handleBackendReady creates the real window against a live server.
...
const startup = Effect.gen(function* () {
  ...
  const trumboAuth = yield* DesktopTrumboAuth.DesktopTrumboAuth;
