...
import { ConnectionCatalogDocument } from "@trumbo-code/client-runtime/platform";
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@trumbo-code/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ TRUMBO_CODE_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "trumbo-code-desktop-connection-catalog-test-",
    });
