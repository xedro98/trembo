...
import "./electron/registerDesktopSchemes.ts";
...
import { resolveRemoteTrumboCliPackageSpec } from "@trumbo-code/ssh/command";
import type { RemoteTrumboRunnerOptions } from "@trumbo-code/ssh/tunnel";
...
import * as DesktopTrumboAuth from "./app/DesktopTrumboAuth.ts";
import * as DesktopTrumboSubscription from "./app/DesktopTrumboSubscription.ts";
...
const resolveDesktopSshCliRunner = (
...
): RemoteTrumboRunnerOptions => {
  const devRemoteEntryPath = Option.getOrUndefined(environment.devRemoteTrumboServerEntryPath);
  ...
  return {
    packageSpec: resolveRemoteTrumboCliPackageSpec({
...
const desktopTrumboAuthLayer = DesktopTrumboAuth.layer.pipe(
  Layer.provideMerge(desktopEnvironmentLayer),
  Layer.provideMerge(NodeServices.layer),
  Layer.provideMerge(ElectronApp.layer),
);

const desktopRuntimeLayer = desktopTrumboAuthLayer.pipe(
  Layer.flatMap((trumboAuthContext) =>
    desktopApplicationLayer.pipe(
      Layer.provideMerge(Layer.succeedContext(trumboAuthContext)),
      Layer.provideMerge(DesktopTrumboSubscription.layer),
      Layer.provideMerge(NodeServices.layer),
