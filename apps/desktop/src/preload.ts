...
contextBridge.exposeInMainWorld("desktopBridge", {
  ...
  trumboAuth: {
    getState: () =>
      ipcRenderer.invoke(IpcChannels.TRUMBO_AUTH_GET_STATE_CHANNEL) as Promise<TrumboAuthState>,
    startSignIn: () =>
      ipcRenderer.invoke(
        IpcChannels.TRUMBO_AUTH_START_SIGN_IN_CHANNEL,
      ) as Promise<TrumboDeviceCodeRequest>,
    cancelSignIn: () =>
      ipcRenderer.invoke(IpcChannels.TRUMBO_AUTH_CANCEL_SIGN_IN_CHANNEL) as Promise<void>,
    signOut: () => ipcRenderer.invoke(IpcChannels.TRUMBO_AUTH_SIGN_OUT_CHANNEL) as Promise<void>,
    refresh: () =>
      ipcRenderer.invoke(IpcChannels.TRUMBO_AUTH_REFRESH_CHANNEL) as Promise<TrumboAuthState>,
    onStateChange: (listener) => {
      const wrappedListener = (_event: Electron.IpcRendererEvent, state: unknown) => {
        if (typeof state !== "object" || state === null) return;
        listener(state as TrumboAuthState);
      };
      ipcRenderer.on(IpcChannels.TRUMBO_AUTH_STATE_CHANGE_CHANNEL, wrappedListener);
      return () =>
        ipcRenderer.removeListener(IpcChannels.TRUMBO_AUTH_STATE_CHANGE_CHANNEL, wrappedListener);
    },
  },
