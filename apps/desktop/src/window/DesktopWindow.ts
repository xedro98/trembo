...
import { clearRendererCacheIfVersionChanged } from "./rendererCache.ts";

const TITLEBAR_HEIGHT = 40;
const TITLEBAR_COLOR = "#01000000"; // #00000000 does not work correctly on Linux
const TITLEBAR_LIGHT_SYMBOL_COLOR = "#1f2937";
const TITLEBAR_DARK_SYMBOL_COLOR = "#f8fafc";
const RENDERER_LOAD_RETRY_DELAYS_MS = [100, 250, 500, 1_000, 2_000] as const;
const RENDERER_RETRYABLE_LOAD_ERROR_CODES = new Set([
  -2, // ERR_FAILED
  -7, // ERR_TIMED_OUT
...
>()("@trumbo-code/desktop/window/DesktopWindow") {}
...
function buildConnectingSplashDataUrl(
  ...
  labelText = "Starting Trumbo Code…",
): string {
...
export function isRetryableRendererLoadFailure(input: {
  ...
  return (
    input.isMainFrame &&
    RENDERER_RETRYABLE_LOAD_ERROR_CODES.has(input.errorCode) &&
...
/** @deprecated Use {@link isRetryableRendererLoadFailure}. */
export const isRetryableDevelopmentRendererLoadFailure = isRetryableRendererLoadFailure;
...
export const make = Effect.gen(function* () {
  ...
  > {
    ...
    let rendererLoadRetryIndex = 0;
    let rendererLoadRetryFiber: Fiber.Fiber<void, never> | undefined;
    const clearRendererLoadRetry = () => {
      if (rendererLoadRetryFiber === undefined) {
        return;
      }
      const retryFiber = rendererLoadRetryFiber;
      rendererLoadRetryFiber = undefined;
    ...
    const loadApplication = () => {
      ...
      void clearRendererCacheIfVersionChanged(environment.appVersion)
        .catch(() => undefined)
        .finally(load);
    };
    const scheduleRendererLoadRetry = () => {
      if (rendererLoadRetryFiber !== undefined || window.isDestroyed()) {
        return undefined;
      }

      const retryIndex = Math.min(rendererLoadRetryIndex, RENDERER_LOAD_RETRY_DELAYS_MS.length - 1);
      const retryInMs = RENDERER_LOAD_RETRY_DELAYS_MS[retryIndex] ?? 2_000;
      rendererLoadRetryIndex += 1;
      rendererLoadRetryFiber = runFork(
    ...
    window.webContents.on("did-finish-load", () => {
      ...
      clearRendererLoadRetry();
      rendererLoadRetryIndex = 0;
      window.setTitle(environment.displayName);
    });
  ...
  return DesktopWindow.of({
    ...
    handleBackendReady: Effect.fn("desktop.window.handleBackendReady")(function* (httpBaseUrl) {
      ...
      const existingWindow = yield* currentMainWindow;
      if (Option.isSome(existingWindow)) {
        // The main window may have opened before the backend finished booting.
        // Reload once the backend is up so the first paint hits a live server.
        const applicationUrl = getDesktopUrl(environment.isDevelopment);
        yield* Effect.sync(() => {
          if (existingWindow.value.isDestroyed()) {
            return;
          }
          const reload = () => {
