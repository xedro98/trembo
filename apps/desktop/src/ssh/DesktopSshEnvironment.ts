...
import * as SshAuth from "@trumbo-code/ssh/auth";
import { discoverSshHosts } from "@trumbo-code/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@trumbo-code/ssh/errors";
import * as SshTunnel from "@trumbo-code/ssh/tunnel";
...
>()("@trumbo-code/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteTrumboRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Trumbo Code window is not available for SSH authentication.";
      break;
