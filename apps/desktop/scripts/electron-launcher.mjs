// This file mostly exists because we want dev mode to say "Trumbo Code (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "Trumbo Code (Dev)" : "Trumbo Code";
export const APP_BUNDLE_ID = isDevelopment
  ? `dev.trumbo.trumbo-code.dev.${devBundleIdSuffix || "local"}`
  : "dev.trumbo.trumbo-code";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["trumbo-dev"] : ["trumbo-code"];
...
// oxlint-disable-next-line trumbo-code/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["TRUMBO_CODE_PORT", environment.TRUMBO_CODE_PORT],
    ["TRUMBO_CODE_HOME", environment.TRUMBO_CODE_HOME],
    ["TRUMBO_CODE_COMMIT_HASH", environment.TRUMBO_CODE_COMMIT_HASH],
    ["TRUMBO_CODE_OTLP_TRACES_URL", environment.TRUMBO_CODE_OTLP_TRACES_URL],
    ["TRUMBO_CODE_OTLP_EXPORT_INTERVAL_MS", environment.TRUMBO_CODE_OTLP_EXPORT_INTERVAL_MS],
    ["TRUMBO_CODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --trumbo-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Trumbo Code name.
