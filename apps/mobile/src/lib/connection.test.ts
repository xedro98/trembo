...
describe("mobile remote connection records", () => {
  ...
  it("removes hosted pairing credentials while keeping the advertised host", () => {
    expect(
      redactPairingCredential(
        "https://platform.trumbo.dev/pair?host=https%3A%2F%2Fdesktop.example&token=bootstrap-token&label=Desktop",
      ),
    ).toBe("https://platform.trumbo.dev/pair?host=https%3A%2F%2Fdesktop.example&label=Desktop");
  });
