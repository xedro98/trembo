...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@trumbo-code/contracts";
...
describe("buildHomeThreadGroups", () => {
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      owner: "trumbo",
      name: "trumbo-code",
      displayName: "Trumbo Code",
      rootPath: "/workspaces/trumbo-code",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/trumbo-code/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/trumbo-code/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "Trumbo Code" })[0];
  ...
  it("targets quick new threads at the group member with the newest thread", () => {
    ...
    const repositoryIdentity = {
      canonicalKey: "github.com/xedro98/trumbo-code",
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:xedro98/trumbo-code.git",
      },
    };
