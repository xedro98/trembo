...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "trumbo-code-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-trumbo-code", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "trumbo-code", specifier: pluginPath }],
