...
Trumbo Code connects directly to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving your editor. Work stays in flow—no more jumping between browser tabs and terminal windows.

## Supported Providers

Trumbo Code works with the platforms your team already uses:
...
- Trumbo Code can suggest titles and descriptions based on your commits
...
1. Install the GitHub CLI on the machine running Trumbo Code:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Trumbo Code and verify GitHub shows as authenticated
...
2. Add these environment variables to the environment running Trumbo Code:
   ```bash
   export TRUMBO_CODE_BITBUCKET_EMAIL="you@example.com"
   export TRUMBO_CODE_BITBUCKET_API_TOKEN="your-token"
   ```
3. Restart Trumbo Code and verify the connection in **Source Control settings**
...
**Git is required** – Trumbo Code uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Trumbo Code (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.
