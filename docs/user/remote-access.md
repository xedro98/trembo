...
Use this when you want to connect to a Trumbo Code server from another device such as a phone, tablet, or separate desktop app.
...
- HTTPS/WSS-compatible endpoints work from `https://platform.trumbo.dev`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://platform.trumbo.dev/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
For `https://platform.trumbo.dev`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
```bash
npx trumbo-code serve --host "$(tailscale ip -4)"
```
...
```bash
npx trumbo-code serve --tailscale-serve
```
...
```bash
npx trumbo-code serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse Trumbo Code on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote Trumbo server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual Trumbo server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.trumbo-code/ssh-launch/<host-key>/`, starts or reuses a remote Trumbo server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. Trumbo Code uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Trumbo Code first checks whether `node` is already available on `PATH`. If it is missing, the launcher tries common non-interactive shell locations and version-manager shims/activation hooks:
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Trumbo Code uses:
...
The hosted web app at `https://platform.trumbo.dev` can save a remote backend in browser local storage from a URL like:

```text
https://platform.trumbo.dev/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
...
Hosted pairing does not proxy traffic through Trumbo Code. The browser still connects directly to the backend URL in the pairing link.
