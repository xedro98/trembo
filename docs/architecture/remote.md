...
This document describes the target architecture for first-class remote environments in Trumbo Code.
...
- Keep the Trumbo server as the execution boundary.
...
Trumbo already has a clean runtime boundary: the client talks to a Trumbo server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Trumbo server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Trumbo server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Trumbo runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Trumbo server instance.
...
How does the client speak WebSocket to a Trumbo server?
...
- best fit for hosted or self-managed remote Trumbo deployments
...
For Trumbo, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Trumbo-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Trumbo server
...
How does a Trumbo server come to exist on the target machine?
...
The user or operator already runs Trumbo on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Trumbo:
...
The recommended Trumbo flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Trumbo availability.
3. Desktop launches or reuses a remote Trumbo server.
...
This is the inverse of remote launch: a local Trumbo server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Trumbo server might be reached through direct `wss`.
...
Trumbo already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Trumbo should not copy that part.

Trumbo already has the right runtime boundary:

- one Trumbo server per environment
- ordinary HTTP/WebSocket between client and environment

So Trumbo should borrow Zed's launch discipline, not its transport protocol.
