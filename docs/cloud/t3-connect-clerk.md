# Trumbo Connect Clerk Setup

Trumbo Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `trumbo-relay` template with the shared
`trumbo-code-relay` audience.

## Application Keys

Trumbo Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
TRUMBO_CODE_CLERK_PUBLISHABLE_KEY=<publishable key>
TRUMBO_CODE_CLERK_JWT_TEMPLATE=<JWT template name>
TRUMBO_CODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
TRUMBO_CODE_RELAY_URL=https://relay.example.com
```
...
should set `TRUMBO_CODE_CLERK_PUBLISHABLE_KEY`, `TRUMBO_CODE_CLERK_JWT_TEMPLATE`,
`TRUMBO_CODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `TRUMBO_CODE_RELAY_URL` before building. EAS preview and
...
deployment defaults.
`vp run --filter trumbo-code-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
1. Create an OAuth application for the Trumbo CLI.
...
5. Set `TRUMBO_CODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
running Trumbo server. The next `t3 serve` or `t3 start` reconciles the relay link and launches the
...
| ------- | -------------------------------- |
| Name    | `trumbo-relay`                   |
| Claims  | `{ "aud": "trumbo-code-relay" }` |

Set `TRUMBO_CODE_CLERK_JWT_TEMPLATE=trumbo-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=trumbo-code-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `TRUMBO_CODE_RELAY_URL` still
...
Local desktop development uses `trumbo-dev://app`, while packaged builds use `trumbo://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `trumbo-dev://app`; the production Clerk instance should only need
`trumbo://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `dev.trumbo.trumbo-code`. To enable native passkeys:

1. Create an explicit macOS App ID for `dev.trumbo.trumbo-code` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.dev.trumbo.trumbo-code`.
...
```dotenv
TRUMBO_CODE_APPLE_TEAM_ID=ABC1234567
TRUMBO_CODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/trumbo-code.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`TRUMBO_CODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `TRUMBO_CODE_PORT` set. Rebuild the signed app
...
TRUMBO_CODE_PORT=13773 \
  "/Applications/Trumbo Code.app/Contents/MacOS/Trumbo Code"
```
...
```sh
codesign --verify --deep --strict "/Applications/Trumbo Code.app"
codesign -d --entitlements :- "/Applications/Trumbo Code.app"
```
...
Approved signed-in users manage Trumbo Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Trumbo Connect controls on the Connections page.

On mobile, signed-out users open **Settings > Trumbo Account** to reach `/settings/waitlist` within the
