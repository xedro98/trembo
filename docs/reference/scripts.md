...
- Dev commands default `TRUMBO_CODE_HOME` to `~/.trumbo-code` — the same shared home the desktop/production app uses. Override with `--home-dir` (see below) to keep dev state separate.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `vp run dev -- --home-dir ~/.trumbo-code-2`
...
- Desktop production windows load the bundled UI from `trumbo://app/index.html` (not a `127.0.0.1` document URL).
...
- Signed macOS builds also require `TRUMBO_CODE_APPLE_TEAM_ID` and
  `TRUMBO_CODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `TRUMBO_CODE_CLERK_PUBLISHABLE_KEY` unless `TRUMBO_CODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
Set `TRUMBO_CODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `13773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `TRUMBO_CODE_DEV_INSTANCE`)
- Example: `TRUMBO_CODE_DEV_INSTANCE=branch-a vp run dev:desktop`

If you want full control instead of hashing, set `TRUMBO_CODE_PORT_OFFSET` to a numeric offset.
