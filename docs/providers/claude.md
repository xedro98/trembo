...
This guide is for people who want to use more than one Claude setup in Trumbo Code.
...
In Trumbo Code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means Trumbo Code uses your normal home directory.
...
Then add another Claude provider in Trumbo Code:
...
Trumbo Code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Trumbo Code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in Trumbo Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Trumbo Code stores the value as a server secret and does not
...
Trumbo Code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in Trumbo Code.
