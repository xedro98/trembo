...
This guide is for people who want to use more than one Codex account in Trumbo Code.
...
- both accounts can see the same Trumbo/Codex sessions
...
In Trumbo Code Settings, name it something obvious:
...
In Trumbo Code Settings, add another Codex provider:
...
Trumbo Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Trumbo Code treats it
