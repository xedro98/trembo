...
- Reads the shared production Trumbo Connect relay URL and Clerk client configuration before packaging clients.
...
## Trumbo Connect relay deployment
...
```sh
vp run --filter trumbo-code-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `TRUMBO_CODE_WEB_ROUTER_URL`: defaults to `https://platform.trumbo.dev`.
- `TRUMBO_CODE_WEB_LATEST_DOMAIN`: defaults to `latest.platform.trumbo.dev`.
- `TRUMBO_CODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.platform.trumbo.dev`.

Required Vercel domains:

- `platform.trumbo.dev`: the router domain users open, updated by stable releases.
- `latest.platform.trumbo.dev`: channel alias updated by stable releases.
- `nightly.platform.trumbo.dev`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__t3code/channel?channel=latest` or
`/__t3code/channel?channel=nightly`; the router stores the
`t3code_web_channel` cookie and rewrites future requests on `platform.trumbo.dev` to
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `platform.trumbo.dev` points at a deployment containing the router
...
- Repository slug source:
  - `TRUMBO_CODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `TRUMBO_CODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
2. Create an explicit App ID for `dev.trumbo.trumbo-code` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Trumbo Connect Clerk Setup](../cloud/t3-connect-clerk.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.dev.trumbo.trumbo-code` and includes
    Associated Domains.
