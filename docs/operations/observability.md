...
Trumbo Code has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.trumbo-code/userdata/logs/server.trace.ndjson`).
...
```bash
export TRUMBO_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export TRUMBO_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export TRUMBO_CODE_OTLP_SERVICE_NAME=trumbo-code-local
```

Optional:

```bash
export TRUMBO_CODE_TRACE_MIN_LEVEL=Info
export TRUMBO_CODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `TRUMBO_CODE_OTLP_*`.

macOS app bundle example:

```bash
TRUMBO_CODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
TRUMBO_CODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
TRUMBO_CODE_OTLP_SERVICE_NAME=trumbo-code-desktop \
...
```bash
tail -f "$TRUMBO_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$TRUMBO_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$TRUMBO_CODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `trumbo-code-local`, `trumbo-code-dev`, or `trumbo-code-desktop`
...
- `TRUMBO_CODE_OTLP_TRACES_URL` was not set
...
- `TRUMBO_CODE_TRACE_FILE`: override trace file path
- `TRUMBO_CODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `TRUMBO_CODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `TRUMBO_CODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `TRUMBO_CODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `TRUMBO_CODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `TRUMBO_CODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `TRUMBO_CODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `TRUMBO_CODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `TRUMBO_CODE_OTLP_SERVICE_NAME`: service name, default `trumbo-code-server`
