...
The screenshot harness runs the real mobile application against three disposable local Trumbo
...
the software keyboard, and supplies deterministic Trumbo Connect discovery rows to the real
...
1. Creates three temporary Trumbo base directories and starts a local server for each on an available
   port.
2. Creates Trumbo Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Trumbo Code review diff.
...
Run Metro separately:
    ...
    pnpm --filter @trumbo-code/mobile showcase
...
The Pending rows use the production offline outbox and point at the real Trumbo Code and React fixture
